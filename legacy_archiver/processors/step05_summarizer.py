...
def call_openai_api(prompt, config):
    ...
    try:
        ...
        # デバッグログ（要約タスクのモデルを優先し、無ければ全体設定へフォールバック）
        api_model = str(
            config.get("ai_task_models", {}).get("summary")
            or config["api_settings"].get("ai_text_model")
            or "gpt-4o"
        )
        print(f"[DEBUG] OpenAI API呼び出し開始: モデル={api_model}, prompt文字数={len(prompt)}")

        client = openai.OpenAI(api_key=api_key)
        effort = str(config.get("ai_task_efforts", {}).get("summary") or "").strip()
        create_kwargs = {
            "model": api_model,
            "messages": [{"role": "user", "content": prompt}],
        }
        if effort:
            # 推論モデル: reasoning_effortを指定し、温度は既定・トークンはmax_completion_tokensを使う
            create_kwargs["reasoning_effort"] = effort
            create_kwargs["max_completion_tokens"] = 1000
            print(f"[DEBUG] OpenAI推論モード: effort={effort}")
        else:
...
def get_codex_exec_config(config):
    ...
    # 配信者ごとの指定を最優先。空なら全体設定(codex_exec)へフォールバック。
    task_model = str(config.get("ai_task_models", {}).get("summary") or "").strip()
    task_effort = str(config.get("ai_task_efforts", {}).get("summary") or "").strip()
    ...
    return CodexExecConfig(
        ...
        model=task_model or model_for_cli_engine(engine, raw),
        effort=task_effort or str(raw.get("effort") or ""),
...
def call_google_api(prompt, config):
    ...
    try:
        ...
        model = genai.GenerativeModel(
            str(config["api_settings"].get("ai_text_model") or "gemini-2.5-flash")
        )
