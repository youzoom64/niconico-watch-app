...
def normalize_audio_for_asr(src_path, dst_path):
    """文字起こし用に音量を平均化した音声を作る。成功でTrue。

    ffmpegのdynaudnormで、小さい喋りを持ち上げ大きい音を抑えて平均化する。
    元音声(プレイヤー用/動画)には触れない。ffmpegが無ければFalseを返し呼び出し側が元音声を使う。
    """
    ffmpeg = find_ffmpeg_exe()
    if not ffmpeg:
        print("ffmpegが見つからないため音量平均化をスキップ")
        return False
    try:
        result = subprocess.run(
            [
                ...
                "-ar", "16000", "-c:a", "libmp3lame", "-b:a", "192k",
                dst_path,
            ],
            capture_output=True, text=True, encoding="utf-8", errors="replace", timeout=600,
        )
        if result.returncode == 0 and os.path.isfile(dst_path) and os.path.getsize(dst_path) > 0:
            print(f"文字起こし用に音量平均化(dynaudnorm)完了: {os.path.basename(dst_path)}")
            return True
        print(f"音量平均化に失敗(exit={result.returncode})、元音声で続行")
        return False
    except Exception as exc:
        print(f"音量平均化でエラー、元音声で続行: {exc}")
        return False
...
def extract_and_split_audio(mp4_path, broadcast_dir, lv_value, parsec=0):
    ...
    try:
        ...
        # HTMLプレイヤーが参照するタイムライン音声は、ズレ0秒でも必ず保存する（無加工）。
        ...
        # 文字起こし専用に、音量を平均化した音声を別途用意する（プレイヤー音声・動画には影響させない）。
        # 配信音声が平均-28dB程度と小さいとVADが無音判定してタイムブロックが空になりやすいので、
        # ffmpegのdynaudnorm(ダイナミックノーマライザ=小さい音を上げ大きい音を抑えて平均化)で底上げする。
        asr_source = silent_audio_path if parsec > 0 else full_audio_path
        asr_audio_path = os.path.join(broadcast_dir, f"{lv_value}_asr_audio.mp3")
        asr_ready = normalize_audio_for_asr(asr_source, asr_audio_path)

        # 180分以下は1ファイルで文字起こしに使う（正規化できたらそれを、無理なら元音声を使う）。
        if total_length <= duration * 1000:
            if asr_ready:
                print(f"180分以下のためチャンク分割を省略（音量平均化音声を使用）: {asr_audio_path}")
                return [(asr_audio_path, 0)]
            print(f"180分以下のためチャンク分割を省略: {asr_source}")
            return [(asr_source, 0)]

        # 180分超は分割。正規化済み音声があればそれを分割対象にする。
        chunk_source_segment = audio_segment
        if asr_ready:
            try:
                chunk_source_segment = AudioSegment.from_mp3(asr_audio_path)
            except Exception as exc:
                print(f"正規化音声の読み込み失敗、元音声で分割: {exc}")
        total_length = len(chunk_source_segment)
        ...
        for start in range(0, total_length, duration * 1000):
            ...
            audio_chunk = chunk_source_segment[start:end]
