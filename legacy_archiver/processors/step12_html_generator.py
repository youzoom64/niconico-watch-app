...
def create_timeline_blocks(
    ...
    try:
        ...
        anonymous_comment_names = build_anonymous_comment_names(comments)
        ...
        for comment in comments:
            ...
            if timeline_block in comment_blocks:
                ...
                raw_user_name = str(comment.get('user_name', '') or '').strip()
                if comment.get('anonymity', False):
                    raw_user_name = anonymous_comment_names.get(
                        str(comment.get('user_id', '') or ''),
                        raw_user_name,
                    )
                elif not raw_user_name:
                    raw_user_id = str(comment.get('user_id', '') or '').strip()
                    if raw_user_id.isdigit():
                        raw_user_name = get_user_nickname_with_cache(raw_user_id) or raw_user_id
                comment_data = {
...
def prepare_comment_ranking(ranking_data, account_dir, lv_value, comments_data=None):
    ...
    try:
        ...
        for rank_data in ranking_rows:
            ...
            if rank_data.get('anonymity', False):
                raw_user_name = anonymous_comment_names.get(str(user_id), raw_user_name)
...
def build_anonymous_comment_names(comments):
    """コメビュと同じく、184匿名IDを最初のコメント番号で固定表示する。"""
    names = {}
    for fallback_no, comment in enumerate(comments or [], 1):
        if not comment.get('anonymity', False):
            continue
        user_id = str(comment.get('user_id', '') or '').strip()
        if not user_id or user_id in names:
            continue
        first_no = str(comment.get('no', '') or '').strip() or str(fallback_no)
        names[user_id] = f"{first_no}コメさん"
    return names
...
def build_virtual_timeline_script(lv_value, timeline_interval=10):
    ...
    (function () {
        ...
        function search(query) {
            ...
            if (!needle) return [];
            const results = [];
            for (const timelineId of ["timeline1", "timeline2"]) {
                const rows = timelineData[timelineId] || [];
                rows.forEach((rowHtml, index) => {
                    ...
                    while (walker.nextNode()) {
                        const text = (walker.currentNode.nodeValue || "").toLocaleLowerCase();
                        let start = 0;
                        while ((start = text.indexOf(needle, start)) !== -1) {
                            results.push({ timelineId, index, occurrence });
                            occurrence += 1;
                            start += needle.length;
                        }
                    }
...
""")
        ...
        <div class="emotion-chart-card">
            ...
            <div class="graph-container">
                <div class="emotion-legend" aria-label="感情分析グラフの凡例">
                    <button type="button" class="emotion-legend-item" data-dataset-index="0" aria-pressed="true"><span class="emotion-legend-line" style="background:#4CAF50"></span>Positive</button>
                    <button type="button" class="emotion-legend-item" data-dataset-index="1" aria-pressed="true"><span class="emotion-legend-line" style="background:#2196F3"></span>Center</button>
                    <button type="button" class="emotion-legend-item" data-dataset-index="2" aria-pressed="true"><span class="emotion-legend-line" style="background:#F44336"></span>Negative</button>
                </div>
...
""")
    ...
    document.addEventListener("DOMContentLoaded", function () {{
        ...
        document.querySelectorAll('.emotion-legend-item').forEach(function(button) {{
            button.addEventListener('click', function() {{
                var datasetIndex = Number(button.dataset.datasetIndex);
                var meta = sentimentChart.getDatasetMeta(datasetIndex);
                meta.hidden = meta.hidden === null
                    ? !sentimentChart.data.datasets[datasetIndex].hidden
                    : !meta.hidden;
                var visible = !meta.hidden;
