...
def process(pipeline_data):
    ...
    try:
        ...
        ncv_xml_path = ""
        ncv_data = fetch_nicolive_program_metadata(lv_value, html_content, account_id)
...
def fetch_nicolive_program_metadata(lv_value, html_content=None, account_id=None):
    """放送ページとAPIから旧NCV XML相当のメタ情報を作る"""
    page_data = parse_embedded_data_from_html(html_content or "")
    api_data = {}
    # HTMLが終了後で owner_id を欠く場合でも、account_id を使ってAPIを必ず試す。
    provider_id = page_data.get("owner_id") or str(account_id or "")
