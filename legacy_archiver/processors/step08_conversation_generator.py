...
{{"conversation": [{{"name": "キャラクター名", "dialogue": "セリフ"}}]}}"""
        ...
        effort = str(config.get("ai_task_efforts", {}).get("conversation") or "").strip()
        create_kwargs = {
            "model": str(
                config.get("ai_task_models", {}).get("conversation")
...
def get_codex_exec_config(config):
    ...
    task_model = str(config.get("ai_task_models", {}).get("conversation") or "").strip()
    task_effort = str(config.get("ai_task_efforts", {}).get("conversation") or "").strip()
