...
.p-context{{margin-top:6px;display:flex;flex-direction:column;gap:4px;border-left:2px solid var(--accent-soft);padding:4px 0 4px 10px}}
.p-context .ctx-seg{{font-size:12.5px;line-height:1.6;color:var(--ink-soft);overflow-wrap:anywhere}}
.p-context .ctx-more{{font-size:11px;color:var(--mute);font-family:"JetBrains Mono",monospace}}
...
const CTX_MAX = 5;
function matchedContext(r){{
  if(!state.query) return '';
  if(state.searchType!=='all'&&state.searchType!=='transcript') return '';
  const q = normalize(state.query);
  const segs = (r.transcript_segments||[]).filter(seg=>normalize(seg).includes(q));
  if(!segs.length) return '';
  const shown = segs.slice(0,CTX_MAX).map(seg=>`<div class="ctx-seg">${{markText(seg)}}</div>`).join('');
  const more = segs.length>CTX_MAX ? `<div class="ctx-more">ほか${{segs.length-CTX_MAX}}件</div>` : '';
  return `<div class="p-context">${{shown}}${{more}}</div>`;
}}
...
function renderPost(r){{
  ...
  // ポップアップは opacity:0 で隠すだけなので、srcを入れたままだと全件ぶん最初に読み込まれる。
  // data-srcで持たせ、ホバー時にhydratePopImageがsrcへ移す。
  const img = r.image_url ? `<img data-src="${{esc(r.image_url)}}" alt="" decoding="async">` : 'summary image';
  ...
  return `<article class="post${{r.history_deleted?' history-deleted':''}}">
    ...
    ${{r.image_url ? `<img class="p-thumb" src="${{esc(r.image_url)}}" alt="要約画像" loading="lazy" decoding="async" width="120" height="68">` : `<div class="p-thumb-empty"></div>`}}
    <div class="p-body">
...
function hydratePopImage(post){{
  const img = post.querySelector('.summary-image img[data-src]');
  if(!img) return;
...
function bindPostCommentFlow(){{
  ...
  document.querySelectorAll('.post').forEach(post => {{
    post.addEventListener('mouseenter', () => {{
      hydratePopImage(post);
      startCommentFlow(post);
    }});
