...
def create_special_user_pages(user_data, broadcast_data, broadcast_dir, lv_value, config=None):
    ...
    try:
        ...
        configured_name = str(user_detail_config.get('display_name') or '').strip()
        user_name = configured_name or user_data['user_name']
        ...
        # ニックネームを取得して上書き
        real_nickname = get_user_nickname_with_cache(user_id) if not configured_name else ''
        if real_nickname:
...
def create_user_detail_page(user_data, broadcast_data, template_dir, output_dir, lv_value, config=None):
    ...
    # 分析テキストを生成（詳細設定を考慮）
    if config:
        analysis_text = generate_analysis_text_with_config(
            user_data['comments'], config, user_id, broadcast_data
        )
    else:
...
def build_prompt_variables(user_detail_config, comments, broadcast_data=None):
    """プロンプト内で置換できる変数。増やしたらGUIの説明も直すこと。"""
    broadcast_data = broadcast_data or {}
    return {
        "{name}": str(user_detail_config.get("display_name") or user_detail_config.get("user_id") or ""),
        "{user_id}": str(user_detail_config.get("user_id") or ""),
        "{title}": str(broadcast_data.get("live_title") or ""),
        "{lv}": str(broadcast_data.get("lv_value") or ""),
        "{broadcaster}": str(broadcast_data.get("broadcaster") or ""),
        "{start_time}": format_start_time(broadcast_data.get("start_time", "")),
        "{comment_count}": str(len(comments or [])),
    }


def apply_prompt_variables(text, variables):
    for key, value in (variables or {}).items():
        text = text.replace(key, value)
    return text


def generate_analysis_text_with_config(comments, config, user_id, broadcast_data=None):
    ...
    if user_detail_config.get("analysis_prompt"):
        ...
        variables = build_prompt_variables(user_detail_config, comments, broadcast_data)
        if engine in {"codex_exec", "claude", "grok"} or ai_model == "openai-gpt4o":
            ai_analysis = generate_ai_analysis(comments, config, user_detail_config, variables)
        elif engine == "gemini" or ai_model == "google-gemini-2.5-flash":
            ai_analysis = generate_gemini_analysis(comments, config, user_detail_config, variables)
...
def generate_ai_analysis(comments, config, user_detail_config, variables=None):
    ...
    try:
        ...
        # プロンプトを構築
        variables = variables or build_prompt_variables(user_detail_config, comments)
        analysis_prompt = apply_prompt_variables(
            user_detail_config.get("analysis_prompt", ""), variables
        )
        # system promptもユーザー設定から取る。未設定なら付けない。
        system_prompt = apply_prompt_variables(
            user_detail_config.get("analysis_system_prompt", ""), variables
        )

        # 送るのは設定したプロンプトとコメントだけ。ここでメタ情報や指示を足さない。
        user_data_text = "\n".join(comment_texts)
        ...
        effort = str(config.get("ai_task_efforts", {}).get("special_user_summary") or "").strip()
        create_kwargs = {
            "model": str(
                config.get("ai_task_models", {}).get("special_user_summary")
                or api_settings.get("ai_text_model")
                or ("gpt-4o" if ai_model == "openai-gpt4o" else "gpt-3.5-turbo")
            ),
            "messages": (
...
def get_codex_exec_config(config, user_detail_config=None):
    ...
    if engine == "claude":
        ...
        command = "grok"
    # 優先順位: スペシャルユーザー個別 > 配信者ごと > 全体設定(codex_exec)
    task_model = str(config.get("ai_task_models", {}).get("special_user_summary") or "").strip()
    task_effort = str(config.get("ai_task_efforts", {}).get("special_user_summary") or "").strip()
    return CodexExecConfig(
        ...
        model=str(
            user_detail_config.get("analysis_ai_model")
            or task_model
            or model_for_cli_engine(engine, raw)
        ),
        effort=str(
            user_detail_config.get("analysis_effort")
            or task_effort
            or raw.get("effort")
            or ""
        ),
...
def generate_gemini_analysis(comments, config, user_detail_config, variables=None):
...
def update_user_list_page(user_data, broadcast_data, template_dir, output_dir, lv_value):
    ...
    if os.path.exists(list_file_path):
        existing_items = load_existing_broadcast_items(list_file_path, exclude_lv=lv_value)
    
    # 新しい放送アイテムを追加
    new_item = generate_broadcast_items(user_data, broadcast_data, lv_value)
    existing_items.append(new_item)
    existing_items = sort_broadcast_items_newest_first(existing_items)
...
def broadcast_item_sort_key(item_html):
    """一覧の項目から放送開始日時を取り出す。取れないものは最古扱い。"""
    import re
    from datetime import datetime as _datetime

    match = re.search(
        r"開始時間:\s*(\d{4})/(\d{2})/(\d{2})\([^)]*\)\s*(\d{2}):(\d{2})", item_html
    )
    if match:
...
def sort_broadcast_items_newest_first(items):
    """放送開始日時の新しい順に並べ替える。"""
    return sorted(items, key=broadcast_item_sort_key, reverse=True)
...
def load_existing_broadcast_items(list_file_path, exclude_lv=None):
    """既存一覧をLV単位で読み込み、重複と更新対象を除外する。"""
    try:
