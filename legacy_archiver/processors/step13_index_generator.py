...
def build_public_archive_index_url(account_id, template=None):
    """配信者別アーカイブ一覧の公開URLを生成する。"""
    value = str(account_id or '').strip()
    if not value:
        raise ValueError('account_id is required')
    pattern = str(template or 'https://warehouse.bitter.jp/niconico/{account_id}/index.html')
    return pattern.format(account_id=value)
...
def process(pipeline_data):
    ...
    try:
        ...
        scope_lv = str(pipeline_data.get('tag_edit_scope_lv') or '').strip()

        print(f"Step13 一覧ページ生成開始: {account_id}")
        if scope_lv:
            print(f"Step13 単体スコープ: {scope_lv} 以外は既存状態を維持する")

        # 1. アカウントディレクトリ検索
        account_dir = find_account_directory(pipeline_data['platform_directory'], account_id)

        # 単体スコープ時に対象外レコードのタグを凍結するため、既存indexのタグを控える
        existing_index_tags = {}
        if scope_lv:
            index_path = Path(account_dir) / 'index.html'
            if index_path.is_file():
                for record in read_archive_records(index_path.read_text(encoding='utf-8')):
                    lv_key = str(record.get('lv') or '').strip()
                    if lv_key:
                        existing_index_tags[lv_key] = normalize_tag_names(record.get('tags') or [])
        ...
        # 単体スコープ: 対象lv以外は既存indexのタグへ戻し、再注釈・再アップロードを起こさない。
        if scope_lv:
            for broadcast in processed_broadcasts:
                lv_key = broadcast.get('lv_value')
                if lv_key != scope_lv and lv_key in existing_index_tags:
                    broadcast['tags'] = list(existing_index_tags[lv_key])
            sync_targets = [
                broadcast for broadcast in processed_broadcasts
                if broadcast.get('lv_value') == scope_lv
            ]
        else:
            sync_targets = processed_broadcasts

        # 4. 各放送ページには管理ブロックだけを追記する。既存HTML全体は再生成しない。
        updated_html_paths = sync_broadcast_html_tags(account_dir, sync_targets, person_aliases)
...
def merge_existing_index_records(account_dir, account_id, current, config):
    ...
    try:
        public_url = build_public_archive_index_url(account_id, public_template)
...
def generate_index_page(
    account_dir, broadcast_list, config, *, account_id='', change_log=None
):
    ...
    broadcaster_name = next(
        (
            str(item.get('broadcaster') or '').strip()
            for item in broadcast_list
            if str(item.get('broadcaster') or '').strip()
            and str(item.get('broadcaster') or '').strip() not in {'不明', account_text}
        ),
        '',
    )
    if not broadcaster_name:
        broadcaster_name = str(config.get('display_name') or account_text or '不明').strip()
    heading = (
        f'{broadcaster_name}(ID {account_text})のリンク一覧'
        if account_text else f'{broadcaster_name}のリンク一覧'
    )
    return step14.generate_modern_list_page(
        account_dir,
        broadcast_list,
        config.get('tags', []),
        document_title=f'{heading} - Archive',
        heading=heading,
        change_log=change_log,
    )
