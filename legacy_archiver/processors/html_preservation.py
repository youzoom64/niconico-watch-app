...
def render_page_tag_block(
    ...
    normalized = normalize_tags(tags)
    stats = stats or {}
    # 一覧(index.html)へ戻るリンク。tag_page_prefix(既定 "../tags/")からアカウント直下を導出。
    index_href = (
        tag_page_prefix.rsplit("tags/", 1)[0] + "index.html"
        if "tags/" in tag_page_prefix else "../index.html"
    )
    back_link = (
        '<a class="archive-page-back" href="' + html.escape(index_href, quote=True) + '"'
        ' style="display:inline-flex;align-items:center;gap:4px;padding:4px 12px;'
        'border-radius:999px;background:#ff6fa8;color:#17131a;text-decoration:none;'
        'font-size:12px;font-weight:700;white-space:nowrap">← リンク一覧</a>'
    )
    links = "".join(
