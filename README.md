...
### 保存済み放送をStep12からGUIで作り直す

1. メインGUIの確認タブを開きます。
2. 対象の配信者と放送を表示します。
3. 対象行を右クリックし、`ローカル処理へ送る`を選びます。複数行を選択してまとめて送ることもできます。
4. 開いたタイムシフトGUIのローカル処理画面で、Step01～11のチェックを外します。
5. `12_html_generator`、`13_index_generator`、`14_modern_list_generator`をチェックします。
6. 公開先も更新する場合だけ`15_lolipop_uploader`をチェックします。
7. `ローカル動画からHTML作成`を押します。

この操作では既存のDBにある文字起こし、コメント、要約、画像などを再利用します。Step02をチェックしない限り、文字起こしはやり直しません。確認タブからローカル動画が見つからない場合は、対象のMP4/MKVをタイムシフトGUIへ直接追加してください。

配信者の保存済み全放送を一括でStep12から処理する場合は、次の補助ツールを使用できます。

```powershell
.\.venv\Scripts\python.exe tools\run_broadcaster_step12_to_end.py `
  --broadcaster-id <配信者ID> `
  --restart
```

このツールは全対象のStep12、全体のStep13・14、HTMLのみのStep15アップロードまで実行します。MP3とスクリーンショットも再送する場合は`--include-media`を付けます。
