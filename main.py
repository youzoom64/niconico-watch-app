...
def run_timeshift_gui(
    ...
    tag_url: str = "",
    special_user_jobs: Sequence[str] = (),
    input_lvs: Sequence[str] = (),
) -> int:
    os.environ[APP_ROLE_ENV] = "timeshift"
    ensure_import_path()
    module = importlib.import_module("app.timeshift_app")
    jobs: list[dict[str, str]] = []
    for value in special_user_jobs or ():
        user_id, _, lv = str(value or "").partition(":")
        if user_id.strip() and lv.strip():
            jobs.append({"user_id": user_id.strip(), "lv": lv.strip()})
    return int(
        module.main(
            initial_urls=list(input_urls),
            initial_files=list(input_files),
            initial_tag_url=tag_url,
            initial_special_user_jobs=jobs,
            initial_lvs=list(input_lvs),
        )
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    timeshift_parser.add_argument(
        ...
        action="append",
        default=[],
        help="動画が残っていない放送をLV指定でローカル処理へ追加する。複数指定可",
    )
    ...
    timeshift_parser.add_argument(
        ...
        action="append",
        default=[],
        help="スペシャル処理へ追加する <ユーザーID>:<LV>。複数指定可",
    )
    timeshift_parser.set_defaults(
        handler=lambda args: run_timeshift_gui(
            args.input_url,
            args.input_file,
            args.tag_url,
            args.special_user_job,
            args.input_lv,
        )
    )
