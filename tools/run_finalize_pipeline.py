...
def main() -> int:
    ...
    try:
        ...
        try:
            archive_result = tracker.archive_monitored_broadcaster_videos(
                args.lv,
                args.broadcaster_id,
                html_file=html_file or None,
                target_dir=args.target_dir or None,
            )
            result["video_archive"] = archive_result
            if archive_result.get("archived"):
                moved = len(archive_result.get("moved") or [])
                print(
                    f"[finalize] archived source videos: files={moved} "
                    f"dir={archive_result.get('archive_dir')}",
                    flush=True,
                )
        except Exception as archive_error:
            # 動画のアーカイブ移動失敗はHTML生成成功を無効化しない。ログのみ残す。
            result["video_archive"] = {"archived": False, "error": str(archive_error)}
            print(f"[finalize] video archive skipped: {archive_error}", flush=True)
            tracker.postprocess_log(
                args.lv,
                "archive_video",
                "WARN",
                f"動画アーカイブ移動に失敗（HTML生成は成功済み）: {archive_error}",
            )
        if args.result_json:
