...
from tracker import niconico_platform_target_root, run_legacy_archiver_steps


def default_broadcast_root() -> Path:
    """設定された保存先に従う。直書きすると保存先を移したときに取り残される。"""
    return niconico_platform_target_root()
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--broadcast-root", type=Path, default=None)
...
def main() -> int:
    args = parse_args()
    broadcast_root = (args.broadcast_root or default_broadcast_root()).resolve()
