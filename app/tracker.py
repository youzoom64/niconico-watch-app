...
SHARED_ENV_PATH = Path(
    os.environ.get("NICONICO_WATCH_SHARED_ENV", str(ROOT / ".env"))
).expanduser()
...
DEFAULT_SPECIAL_USER_ANALYSIS_PROMPT = (
    "次の文章はある配信についた、あるリスナーのコメントです\n"
    "要約して分析してください"
)
...
DEFAULT_SUNO_SUBSCRIPTION_MODELS = ["V5_5", "V5", "V4_5PLUS", "V4_5ALL", "V4_5", "V4"]
...
def suno_cli_dir() -> Path:
    value = str(os.environ.get("NICONICO_SUNO_CLI_DIR") or "").strip()
    return Path(value).expanduser() if value else ROOT / "tools" / "suno_cli"
...
CREATE TABLE IF NOT EXISTS special_users (
    ...
    html_generation_enabled INTEGER NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS special_user_html_uploads (
    user_id TEXT NOT NULL,
    lv TEXT NOT NULL,
    uploaded_at TEXT NOT NULL,
    remote_directory TEXT,
    PRIMARY KEY (user_id, lv)
);
...
CREATE TABLE IF NOT EXISTS monitored_broadcasters (
    ...
    html_schedule_enabled INTEGER NOT NULL DEFAULT 0,
    html_schedule_start TEXT NOT NULL DEFAULT '05:00',
    html_schedule_end TEXT NOT NULL DEFAULT '',
    html_backlog_scan_enabled INTEGER NOT NULL DEFAULT 1,
    html_schedule_max_per_window INTEGER NOT NULL DEFAULT 0,
    video_archive_enabled INTEGER NOT NULL DEFAULT 1,
...
FINALIZE_SCHEDULE_DEFAULT_START = "05:00"
FINALIZE_BACKLOG_MAX_ATTEMPTS = 3


def parse_schedule_clock(value: Any) -> clock_time | None:
    """``'5:00'`` や ``'05:00'`` を時刻へ。空文字と不正値はNone（時刻指定なし）。"""
    text = str(value or "").strip()
    if not text:
        return None
...
def finalize_schedule_from_row(row: Any) -> dict[str, Any]:
    """監視配信者の行から、HTML生成の実行枠設定だけを取り出す。"""
    values = dict(row) if row is not None else {}
    return {
        "enabled": bool(values.get("html_schedule_enabled") or 0),
        "start": str(values.get("html_schedule_start") or FINALIZE_SCHEDULE_DEFAULT_START),
        "end": str(values.get("html_schedule_end") or ""),
        "backlog_scan_enabled": bool(
            values.get("html_backlog_scan_enabled")
            if values.get("html_backlog_scan_enabled") is not None
            else 1
        ),
        "max_per_window": max(0, int(values.get("html_schedule_max_per_window") or 0)),
    }


def finalize_schedule_window_is_open(schedule: dict[str, Any], at: datetime | None = None) -> bool:
    """今この瞬間に、その配信者のHTML生成を始めてよいか。

    終了時刻が空なら開始時刻以降はずっと開いている。開始が終了より後なら日跨ぎ枠。
    """
    if not schedule.get("enabled"):
        return True
    start = parse_schedule_clock(schedule.get("start"))
    if start is None:
        return True
    end = parse_schedule_clock(schedule.get("end"))
    current = (at or datetime.now()).time()
    if end is None or end == start:
        # 終了なし。開始時刻を過ぎた後は、キューが空になるまで走らせる。
...
def monitored_broadcaster_finalize_schedules(
    conn: sqlite3.Connection | None = None,
) -> dict[str, dict[str, Any]]:
    def read(active: sqlite3.Connection) -> dict[str, dict[str, Any]]:
        rows = active.execute(
            """
            SELECT broadcaster_id, html_generation_enabled, html_schedule_enabled,
                   html_schedule_start, html_schedule_end,
                   html_backlog_scan_enabled, html_schedule_max_per_window
            FROM monitored_broadcasters
            """
        ).fetchall()
        ...
        for row in rows:
            ...
            if not broadcaster_id:
                continue
            schedule = finalize_schedule_from_row(row)
            schedule["html_generation_enabled"] = bool(row["html_generation_enabled"])
            schedules[broadcaster_id] = schedule
        return schedules
...
def finalize_schedule_allows_now(
    broadcaster_id: str,
    *,
    at: datetime | None = None,
    schedules: dict[str, dict[str, Any]] | None = None,
) -> bool:
    """未登録の配信者やスケジュールOFFの配信者は、常に即時実行を許す。"""
    broadcaster_id = str(broadcaster_id or "").strip()
    if not broadcaster_id:
        return True
    if schedules is None:
        schedules = monitored_broadcaster_finalize_schedules()
    schedule = schedules.get(broadcaster_id)
    if schedule is None:
        return True
    return finalize_schedule_window_is_open(schedule, at)


def format_finalize_schedule_label(row: Any) -> str:
    """一覧表示用の ``05:00-09:00`` / ``05:00-`` 表記。枠なしは空文字。"""
    schedule = finalize_schedule_from_row(row)
    if not schedule["enabled"]:
        return ""
    start = parse_schedule_clock(schedule["start"])
    if start is None:
        return ""
    end = parse_schedule_clock(schedule["end"])
...
def monitored_broadcaster_video_archive_enabled(broadcaster_id: str) -> bool:
    """自動処理で録画動画をHTMLフォルダ配下のarchiveへ集約するか（既定ON）。"""
    broadcaster_id = str(broadcaster_id or "").strip()
    if not broadcaster_id:
        return False
    with connect() as conn:
        row = conn.execute(
            """
            SELECT video_archive_enabled
            FROM monitored_broadcasters
            WHERE broadcaster_id = ?
            """,
            (broadcaster_id,),
        ).fetchone()
    return True if row is None else bool(row["video_archive_enabled"])


def archive_monitored_broadcaster_videos(
    lv: str,
    broadcaster_id: str,
    *,
    ...
    """自動処理専用: 設定が有効な配信者の処理済み録画動画をarchiveへ移動する。

    タイムシフトGUIの経路とは独立しており、実在するソース動画だけを対象にする。
    録画区間DBに残る旧世代パス（既に移動・削除済み）は自動的に除外する。
    """
    if not monitored_broadcaster_video_archive_enabled(broadcaster_id):
        return {"archived": False, "reason": "disabled"}
    with connect() as conn:
        rows = conn.execute(
            "SELECT DISTINCT source_path FROM recording_segments WHERE lv = ?",
            (str(lv or "").strip().lower(),),
        ).fetchall()
    existing: list[Path] = []
    seen: set[str] = set()
    for row in rows:
        raw = str(row["source_path"] or "").strip()
        if not raw:
            continue
        path = Path(raw)
        if not path.is_file():
    ...
    if not existing:
        return {"archived": False, "reason": "no_source_video"}
    # 同一録画区間の .ts/.mp4 重複は変換済みMP4を優先して1件に絞る。
    videos = deduplicate_recording_paths(existing)
    result = archive_processed_video_files(
        lv,
        videos,
...
def existing_recording_video_paths_by_lv(lvs: list[str]) -> dict[str, list[Path]]:
    ...
    # 既定値は旧バージョンの保存先を指したままなので、現在の保存先も必ず走査する。
    scan_roots: list[Path] = []
    for root in (slnico_storage_root(), DEFAULT_SLNICO_RECORDING_ROOT):
        if root and root.is_dir() and root not in scan_roots:
...
def reserve_finalize_queue_item(
    ...
    *,
    ...
    ``created_at`` はFIFOの並び順そのものなので、後追いで積む過去放送には
    その放送が終わった時刻を渡す。省略時は現在時刻。
    """
    current_time = now_micro()
    created_time = str(created_at or "").strip() or current_time
...
def finalize_queue_has_work(at: datetime | None = None) -> bool:
    """今すぐ着手できる仕事があるか。実行枠の外で待たせている分は数えない。"""
    with connect() as conn:
        rows = conn.execute(
            """
        ...
        if not rows:
            return False
        if any(str(row["status"] or "") == "running" for row in rows):
            return True
        schedules = monitored_broadcaster_finalize_schedules(conn)
    return any(
        finalize_schedule_allows_now(str(row["broadcaster_id"] or ""), at=at, schedules=schedules)
        for row in rows
    )
...
def claim_next_finalize_queue_item(at: datetime | None = None) -> dict[str, Any] | None:
    ...
    with connect() as conn:
        ...
        if not candidates:
            conn.commit()
            return None
        # 実行枠が閉じている配信者の分は飛ばし、開いている中で一番古い1件を取る。
        schedules = monitored_broadcaster_finalize_schedules(conn)
        row = next(
            (
                candidate
                for candidate in candidates
                if finalize_schedule_allows_now(
                    str(candidate["broadcaster_id"] or ""),
                    at=at,
                    schedules=schedules,
                )
            ),
...
def set_system_sleep_suppressed(suppressed: bool) -> bool:
    """Windowsのスリープだけ止める。画面は消えていい。"""
    if os.name != "nt":
        return False
    flags = ES_CONTINUOUS | ES_SYSTEM_REQUIRED if suppressed else ES_CONTINUOUS
    try:
...
def keep_system_awake() -> Any:
    """後処理中にPCが寝るとGPU処理ごと固まるので、走っている間だけ抑止する。"""
    suppressed = set_system_sleep_suppressed(True)
    if not suppressed:
        postprocess_log("", "dispatcher", "WARN", "スリープ抑止を設定できない")
    try:
        yield suppressed
    finally:
        if suppressed:
            set_system_sleep_suppressed(False)
...
def start_finalize_pipeline_after_recording_end(
    *,
    ...
    if start_finalize_dispatcher_process():
        return
    schedule = monitored_broadcaster_finalize_schedules().get(str(broadcaster_id or "").strip())
    if schedule is not None and schedule.get("enabled") and not finalize_schedule_window_is_open(schedule):
        postprocess_log(
            lv,
            "dispatcher",
            "INFO",
            "実行枠の外なのでHTML生成はキューで待機",
            {"schedule": f"{schedule.get('start')}-{schedule.get('end') or ''}"},
        )
        return
    postprocess_log(lv, "dispatcher", "ERROR", "終了処理キューディスパッチャーを起動できない")


FINALIZE_BACKLOG_SCAN_INTERVAL_SECONDS = 300.0
_FINALIZE_BACKLOG_SCAN_LOCK = threading.Lock()
_FINALIZE_BACKLOG_LAST_SCAN_MONOTONIC = 0.0


def finalize_backlog_candidates(at: datetime | None = None) -> list[dict[str, Any]]:
    """今すぐ積んでよい未処理放送を、放送が終わった順に返す。

    状態カラムは信用しない。HTMLが実際に無く、録画素材が実際に残っている放送だけを
    対象にする。ここを緩めると、素材の消えた過去放送で失敗を量産する。
    """
    schedules = monitored_broadcaster_finalize_schedules()
    targets = {
        broadcaster_id: schedule
        for broadcaster_id, schedule in schedules.items()
        if schedule.get("html_generation_enabled")
        and schedule.get("enabled")
        and schedule.get("backlog_scan_enabled")
        and finalize_schedule_window_is_open(schedule, at)
    }
    if not targets:
        return []
    ...
    with connect() as conn:
        rows = conn.execute(
            f"""
            SELECT lv, broadcaster_id, broadcaster_name, target_dir, updated_at
            FROM recording_jobs
            WHERE broadcaster_id IN ({placeholders})
              AND status NOT IN ('launched', 'recording', 'finalizing')
              AND lv NOT IN (
                  SELECT lv FROM finalize_queue
                  WHERE status != 'failed' OR attempts >= ?
              )
            ORDER BY updated_at, lv
            """,
            (*targets.keys(), FINALIZE_BACKLOG_MAX_ATTEMPTS),
        ).fetchall()
    active_lvs = {str(job.get("lv") or "").strip() for job in running_live_recording_jobs()}
    taken: dict[str, int] = {}
    candidates: list[dict[str, Any]] = []
    for row in rows:
        lv = str(row["lv"] or "").strip()
        broadcaster_id = str(row["broadcaster_id"] or "").strip()
        if not lv or lv in active_lvs:
            continue
        max_per_window = int(targets[broadcaster_id].get("max_per_window") or 0)
        if max_per_window and taken.get(broadcaster_id, 0) >= max_per_window:
            continue
        if not find_recording_segment_files(lv):
            continue
        if existing_generated_archive_html(lv) is not None:
            continue
        taken[broadcaster_id] = taken.get(broadcaster_id, 0) + 1
        candidates.append(
            {
                "lv": lv,
                "broadcaster_id": broadcaster_id,
                ...
                "ended_at": str(row["updated_at"] or ""),
            }
        )
    return candidates


def enqueue_backlog_finalize_items(at: datetime | None = None) -> list[dict[str, Any]]:
    """未処理放送をFIFOキューへ積む。並び順は放送が終わった順。"""
    enqueued: list[dict[str, Any]] = []
    for candidate in finalize_backlog_candidates(at):
        lv = str(candidate["lv"])
        target_dir = candidate["target_dir"] or str(
            broadcast_target_dir(lv, load_config(), broadcaster_id=candidate["broadcaster_id"] or None)
        )
        with connect() as conn:
            reserved = reserve_finalize_queue_item(
                conn,
...
def maybe_enqueue_backlog_finalize_items(force: bool = False) -> list[dict[str, Any]]:
    """GUIのタイマーから叩く入口。走査はディスク全体を舐めるので間隔を空ける。"""
    global _FINALIZE_BACKLOG_LAST_SCAN_MONOTONIC
    with _FINALIZE_BACKLOG_SCAN_LOCK:
        current = time.monotonic()
        if (
            not force
            and _FINALIZE_BACKLOG_LAST_SCAN_MONOTONIC
            and current - _FINALIZE_BACKLOG_LAST_SCAN_MONOTONIC < FINALIZE_BACKLOG_SCAN_INTERVAL_SECONDS
        ):
            return []
        _FINALIZE_BACKLOG_LAST_SCAN_MONOTONIC = current
    try:
        enqueued = enqueue_backlog_finalize_items()
    except Exception as exc:
        postprocess_log("", "dispatcher", "WARN", f"未処理放送の走査に失敗: {type(exc).__name__}: {exc}")
        return []
    if enqueued:
        start_finalize_dispatcher_process()
    return enqueued
...
def start_recordings_for_monitored_broadcaster_api(
    ...
    for broadcaster_id, broadcaster in monitored.items():
        ...
        for live in lives:
            ...
            record_live_watch_count(conn, item.get("lv"), item.get("watch_count"))
    ...
    *,
    ...
    with segment_transcription_lock(lv):
        ...
        try:
            ...
            if progress_callback:
                progress_callback(
                    f"文字起こし開始 engine={settings['engine']} model={model} device=auto compute=default"
                )
            common_kwargs = {
                "model_size": model,
...
def build_legacy_archiver_config(config: Config | None = None) -> dict[str, Any]:
    ...
    legacy_config = {
        ...
        "music_settings": {
            "provider": str(config.suno_music_provider or "api"),
    ...
    legacy_config["special_users_config"]["users"] = {
        str(row["user_id"]): {
            ...
            # 未設定なら既定文を使う。分析を止めるのは analysis_enabled 側。
            "analysis_prompt": (
                str(row["analysis_prompt"] or "").strip()
                or DEFAULT_SPECIAL_USER_ANALYSIS_PROMPT
            ),
            "analysis_system_prompt": str(row["analysis_system_prompt"] or "").strip(),
...
def apply_monitored_broadcaster_feature_overrides(lv: str, legacy_config: dict[str, Any]) -> dict[str, Any]:
    ...
    # 配信者ごとのAI生成モデル/推論(effort)をタスク別に持つ。空ならグローバル設定へフォールバック。
    row_keys = set(row.keys())

    def _row_text(column):
        return str((row[column] if column in row_keys else "") or "").strip()

    legacy_config["ai_task_models"] = {
        "summary": _row_text("summary_model"),
        "conversation": _row_text("conversation_model"),
        "special_user_summary": _row_text("special_user_summary_model"),
    }
    legacy_config["ai_task_efforts"] = {
        "summary": _row_text("summary_effort"),
        "conversation": _row_text("conversation_effort"),
        "special_user_summary": _row_text("special_user_summary_effort"),
    }
    return legacy_config
...
def parse_archive_tag_entries(value: Any) -> dict[str, Any]:
    ...
    # 認識支援文字(recognition)はWhisperのhotwords専用。
    # 人物ハイライトの別名には流用しない（短い支援文字が本文全体に誤マッチする事故を防ぐ）。
    # 別名は index_person_aliases.json（「別名」欄）からのみ取得する。
...
# これらが失敗したら後続へ進めない。素材が無い、あるいはHTMLが出来ていない状態で
# 一覧更新やアップロードへ進むと、壊れたページを公開してしまうため。
# ここに入っていないstep（感情/単語/要約/音楽/画像/会話/スクショなど）は
# 付加情報であり、失敗しても残りを実行する。
LEGACY_STEP_ABORT_ON_FAILURE = {
...
def run_legacy_archiver_steps(
    ...
    *,
    ...
    single_broadcast_scope: bool = False,
    progress_callback: Callable[[str], None] | None = None,
) -> dict[str, Any]:
    ...
    if single_broadcast_scope:
        # 単体放送の編集反映: step13で対象lv以外の個別HTML/indexレコードを触らない。
        pipeline_data["tag_edit_scope_lv"] = lv
    ...
    try:
        ...
        for step_name in step_names:
            ...
            try:
                ...
                results.setdefault("failed_steps", []).append(step_name)
                if step_name in LEGACY_STEP_ABORT_ON_FAILURE:
                    # 素材そのものが無い/HTMLが出来ていない状態で後続へ進むと、
                    # 壊れたページを公開してしまう。ここだけは従来通り中断する。
                    raise
                # 付加情報を作るステップの失敗で全体を捨てない。
                # 画像1枚が作れないだけでHTML生成やアップロードまで落ちるのを防ぐ。
                postprocess_log(
                    lv,
                    "archive_steps",
                    "WARN",
                    f"legacy step失敗を許容して継続: {step_name}",
                )
                continue
...
def transcription_repeat_key(segment: dict[str, Any]) -> str:
    return re.sub(r"\s+", "", str(segment.get("text") or "")).casefold()


def drop_repeated_transcription_rows(
    segments: list[dict[str, Any]],
    *,
    min_run: int = 3,
) -> tuple[list[dict[str, Any]], int]:
    """同一テキストが連続する塊(Whisperのハルシネーションループ)を先頭1件だけ残す。"""
    threshold = max(2, int(min_run))
    kept: list[dict[str, Any]] = []
    dropped = 0
    run_start = 0
    while run_start < len(segments):
        key = transcription_repeat_key(segments[run_start])
        run_end = run_start + 1
        if key:
            while run_end < len(segments) and transcription_repeat_key(segments[run_end]) == key:
                run_end += 1
        run = segments[run_start:run_end]
        if key and len(run) >= threshold:
            kept.append(run[0])
            dropped += len(run) - 1
        else:
            kept.extend(run)
    ...
    *,
    ...
    try:
        ...
        raw_segments, info = model.transcribe(
            str(audio),
            vad_filter=True,
            # 無音/BGM区間で言語モデルが暴走し同じ文を延々と出す(ハルシネーションループ)のを防ぐ。
            # condition_on_previous_text=True だと前の窓の出力が次の窓のプロンプトになり、
            # 一度ループに入ると放送終わりまで伝播する。
    ...
    rows, dropped_repeats = drop_repeated_transcription_rows(rows)
    if dropped_repeats:
        postprocess_log(
            lv,
            "transcribe",
            "WARN",
            f"繰り返しハルシネーションを除去 dropped={dropped_repeats} remaining={len(rows)}",
            {"dropped_segments": dropped_repeats, "remaining_segments": len(rows)},
        )
    ...
    *,
    ...
    try:
        ...
        if not helper_python.exists():
            raise RuntimeError(
                "WhisperX専用.venvがありません。install_whisperx.batを実行してください: "
                f"{helper_python}"
            )
...
def special_user_html_dir(user_id: str, config: Config | None = None) -> Path:
    """スペシャルユーザー監視で作るHTMLの置き場。配信者ではなくユーザー自身の配下。"""
    config = config or load_config()
    return niconico_platform_target_root(config) / str(user_id).strip() / "special_user"


def build_special_user_broadcast_data(conn: sqlite3.Connection, lv: str) -> dict[str, Any]:
    """録画が無い放送用に、metaからstep11が使うbroadcast_dataを組み立てる。"""
    row = conn.execute(
        "SELECT * FROM broadcast_archive_meta WHERE lv = ?",
        (lv,),
    ).fetchone()
    if row is None:
        raise ValueError(f"broadcast_archive_meta が無い: {lv}")
    meta = dict(row)
    return {
        "lv_value": lv,
        "live_num": lv[2:] if lv.startswith("lv") else lv,
        ...
        "begin_time": meta.get("begin_time"),
        "open_time": str(meta.get("open_time") or ""),
        # start_timeがNULLの放送があるので、開始扱いできる値を順に拾う。
        "start_time": str(
            meta.get("start_time")
            or meta.get("begin_time")
            or meta.get("open_time")
            or ""
        ),
        "end_time": str(meta.get("end_time") or ""),
        "server_time": str(meta.get("server_time") or ""),
        "time_diff_seconds": meta.get("time_diff_seconds") or 0,
        "elapsed_time": "",
        "comment_count": "",
        "watch_count": "",
        "default_community": "",
        ...
        "ncv_xml_path": "",
        "platform_xml_path": "",
        "source": "ndgr_no_recording",
        "timestamp": datetime.now().isoformat(),
    }


def import_step11_module():
    """step11をライブラリとして読み込む。パイプライン外から個別に呼ぶため。"""
    legacy_dir = ROOT / "legacy_archiver"
    for path in (str(legacy_dir), str(legacy_dir / "processors"), str(ROOT / "app")):
        if path not in sys.path:
            sys.path.insert(0, path)
    return importlib.import_module("step11_special_user_html_generator")


def special_user_html_generation_targets_for_lv(
    conn: sqlite3.Connection, lv: str
) -> list[dict[str, Any]]:
    # ヒット記録の有無ではなく、その放送に実際にコメントが残っているかで判定する。
    # ヒット記録はコメント監視タブが動いていた放送にしか作られないため、
    # それを条件にすると確認タブの一覧に出ているのに処理できない放送が出る。
    rows = conn.execute(
        """
        SELECT u.user_id, u.label, MIN(c.id) AS first_id
        FROM archive_comments c
        JOIN special_users u ON u.user_id = c.user_id
        WHERE c.lv = ?
          AND u.enabled = 1
          AND u.html_generation_enabled = 1
        GROUP BY u.user_id, u.label
        ORDER BY first_id
        """,
        (lv,),
    ).fetchall()
    return [{"user_id": row["user_id"], "label": row["label"]} for row in rows]


def generate_special_user_html_for_lv(
    lv: str,
    config: Config | None = None,
    include_analysis: bool = True,
    force_overwrite_existing_html: bool = False,
) -> list[dict[str, Any]]:
    """HTML生成ONのスペシャルユーザーについて、一覧と個別ページを作る。

    出力先は配信者側(step11がパイプラインで作る場所)ではなく、
    <target>/platform/niconico/<user_id>/special_user/ 配下。
    """
    lv = str(lv).strip()
    config = config or load_config()
    results: list[dict[str, Any]] = []
    with connect() as conn:
        targets = special_user_html_generation_targets_for_lv(conn, lv)
        if not targets:
            return results
        broadcast_data = build_special_user_broadcast_data(conn, lv)

    # legacy_archiver をsys.pathに乗せてから archive_db を読む
    step11 = import_step11_module()
    from archive_db import load_comments_payload

    template_dir = str(ROOT / "legacy_archiver" / "templates")
    legacy_config = build_legacy_archiver_config(config)
    if not include_analysis:
        # プロンプトを空にすると step11 はAI呼び出しをせず基本統計だけ出す。
        for user_config in (legacy_config.get("special_users_config", {}).get("users") or {}).values():
            user_config["analysis_prompt"] = ""
    comments_data = load_comments_payload(lv)
    user_ids = [str(target["user_id"]) for target in targets]
    labels = {str(target["user_id"]): str(target.get("label") or "") for target in targets}
    found_users = step11.find_special_users_in_comments_data(comments_data, user_ids)

    for user_data in found_users:
        user_id = str(user_data["user_id"])
        label = labels.get(user_id, "").strip()
        if label:
            user_data["user_name"] = label
        output_dir = special_user_html_dir(user_id, config)
        try:
            output_dir.mkdir(parents=True, exist_ok=True)
            step11.copy_static_files(template_dir, str(output_dir))
            # step12と同じ扱い。既存の個別HTMLは正本として保持し、
            # 明示的な作り直しのときだけ上書きする。分析済みの内容を壊さない。
            detail_path = output_dir / f"{user_id}_{lv}_detail.html"
            preserved = detail_path.is_file() and not force_overwrite_existing_html
            if preserved:
                print(f"既存の個別HTMLを保持: {detail_path}")
            else:
                step11.create_user_detail_page(
                    user_data, broadcast_data, template_dir, str(output_dir), lv, legacy_config
                )
            step11.update_user_list_page(
                user_data, broadcast_data, template_dir, str(output_dir), lv
            )
            apply_special_user_design(user_id, output_dir, lv)
            results.append(
                {
                    "user_id": user_id,
                    "lv": lv,
                    "output_dir": str(output_dir),
                    "comments": len(user_data.get("comments") or []),
                    "status": "preserved" if preserved else "generated",
                    "error": "",
                }
            )
        except Exception as exc:
...
def apply_special_user_design(user_id: str, output_dir: Path, lv: str = "") -> None:
    """生成したHTMLへ、そのユーザーのデザイン設定をCSSとして差し込む。"""
    try:
        import special_user_design
    except ImportError:
        return
    list_path = output_dir / f"{user_id}_list.html"
    if list_path.is_file():
        try:
            special_user_design.apply_design_to_html(
                list_path,
                user_id,
                special_user_design.PAGE_KIND_LINK,
            )
        except Exception as exc:
            print(f"デザイン適用に失敗: {list_path.name}: {exc}")
    targets = []
    if lv:
        targets.append(output_dir / f"{user_id}_{lv}_detail.html")
    else:
        targets.extend(sorted(output_dir.glob(f"{user_id}_lv*_detail.html")))
    for path in targets:
        if path.is_file():
            try:
                special_user_design.apply_design_to_html(path, user_id)
            except Exception as exc:
                print(f"デザイン適用に失敗: {path.name}: {exc}")


def sync_special_user_list_page(user_id: str, config: Config | None = None) -> dict[str, Any]:
    """一覧HTMLを、実在する個別HTMLに合わせて補完する。

    一覧に載っていない個別HTMLがあるとリンクから辿れないので、
    DBのコメントと放送メタから項目を作って追記する。AIは呼ばない。
    """
    user_id = str(user_id or "").strip()
    config = config or load_config()
    source_root = special_user_html_dir(user_id, config)
    if not user_id or not source_root.is_dir():
        return {"added": [], "reason": "ディレクトリなし"}

    detail_lvs: list[str] = []
    for path in sorted(source_root.glob(f"{user_id}_lv*_detail.html")):
        name = path.name
        lv = name[len(user_id) + 1 : -len("_detail.html")]
        if re.fullmatch(r"lv\d+", lv):
            detail_lvs.append(lv)
    if not detail_lvs:
        return {"added": [], "reason": "個別HTMLなし"}

    list_path = source_root / f"{user_id}_list.html"
    listed: set[str] = set()
    if list_path.is_file():
        listed = set(re.findall(r"lv\d+", list_path.read_text(encoding="utf-8", errors="ignore")))
    missing = [lv for lv in detail_lvs if lv not in listed]
    if not missing:
        return {"added": []}
    ...
    with connect() as conn:
        ...
        display_name = str(label_row["label"]) if label_row else user_id
        for lv in missing:
            try:
                broadcast_data = build_special_user_broadcast_data(conn, lv)
            except ValueError:
                continue
            comments_data = load_comments_payload(lv)
            found = step11.find_special_users_in_comments_data(comments_data, [user_id])
            if not found:
                continue
            user_data = found[0]
            user_data["user_name"] = display_name
            step11.update_user_list_page(
                user_data, broadcast_data, template_dir, str(source_root), lv
            )
            added.append(lv)
    return {"added": added}


def special_user_uploaded_lvs(conn: sqlite3.Connection, user_id: str) -> set[str]:
    """公開済みとして記録されているLV。"""
    rows = conn.execute(
        "SELECT lv FROM special_user_html_uploads WHERE user_id = ?",
        (str(user_id or "").strip(),),
    ).fetchall()
    return {str(row["lv"]) for row in rows if str(row["lv"] or "").strip()}


def special_user_html_uploaded(user_id: str, lv: str) -> bool:
    user_id = str(user_id or "").strip()
    lv = str(lv or "").strip()
    ...
    with connect() as conn:
        row = conn.execute(
            "SELECT 1 FROM special_user_html_uploads WHERE user_id = ? AND lv = ? LIMIT 1",
            (user_id, lv),
        ).fetchone()
    return row is not None


def credentials_api_healthy(url: str = "http://127.0.0.1:8796/health") -> bool:
    try:
...
def find_credentials_api_python() -> Path | None:
    """パスワード管理APIを動かせるPythonを探す。

    APIはuvicornを要求するが、このアプリのvenvには入っていない。
    共有ツール側の起動バッチが使っている共通venvを既定にする。
    """
    candidates: list[Path] = []
    env_value = str(os.environ.get("NICONICO_CREDENTIALS_API_PYTHON") or "").strip()
    if env_value:
...
def find_credentials_api_workdir() -> Path | None:
    """scripts.password_manager.api_main を import できる作業ディレクトリを探す。

    このモジュールはアプリ配下ではなく共有ツール側に置かれている。
    """
    candidates: list[Path] = []
    env_value = str(os.environ.get("NICONICO_CREDENTIALS_API_WORKDIR") or "").strip()
    if env_value:
        candidates.append(Path(env_value))
    candidates.append(ROOT)
    for candidate in candidates:
        try:
            if (candidate / "scripts" / "password_manager" / "api_main.py").is_file():
                return candidate
        except OSError:
            continue
    return None
...
def ensure_credentials_api_running(
    config: Config | None = None,
    progress_callback=None,
    health_url: str = "http://127.0.0.1:8796/health",
) -> None:
    """止まっていればパスワード管理APIを非表示で起動する。step15と同じ扱い。"""
    if credentials_api_healthy(health_url):
        return
    config = config or load_config()
    python_exe = find_credentials_api_python()
    if python_exe is None:
        raise RuntimeError(
            "uvicornを持つPythonが見つかりません。"
            "NICONICO_CREDENTIALS_API_PYTHON で指定してください"
        )
    workdir = find_credentials_api_workdir()
    if workdir is None:
        raise RuntimeError(
            "scripts/password_manager/api_main.py が見つかりません。"
            "NICONICO_CREDENTIALS_API_WORKDIR で場所を指定してください"
        )
    if progress_callback:
        progress_callback(f"パスワード管理APIを起動します: {workdir}")
    subprocess.Popen(
        [str(python_exe), "-m", "scripts.password_manager.api_main"],
        cwd=str(workdir),
    ...
    while time.monotonic() < deadline:
        if credentials_api_healthy(health_url):
            if progress_callback:
                progress_callback("パスワード管理APIを起動しました")
            return
        time.sleep(0.25)
    raise RuntimeError("起動を待ちましたが応答がありません")


def upload_special_user_html(
    user_id: str,
    lvs: list[str],
    config: Config | None = None,
    progress_callback=None,
    ...
    送信元は <target>/platform/niconico/<user_id>/special_user、
    公開先は niconico/<user_id>/special_user。配信者アーカイブとは別ディレクトリ。
    """
    user_id = str(user_id or "").strip()
    lvs = [str(value).strip() for value in (lvs or []) if str(value).strip()]
    if not user_id or not lvs:
        return {"uploaded": False, "reason": "対象がありません"}

    config = config or load_config()
    source_root = special_user_html_dir(user_id, config)
    if not source_root.is_dir():
        return {"uploaded": False, "reason": f"生成物のディレクトリがありません: {source_root}"}

    # 個別HTMLに合わせて一覧を補完してから送る。毎回上書きで公開する。
    synced = sync_special_user_list_page(user_id, config)
    if synced.get("added") and progress_callback:
        progress_callback(f"一覧に{len(synced['added'])}放送を追記: {', '.join(synced['added'])}")

    relative_paths: list[str] = []
    missing_lvs = [
        lv for lv in lvs if not (source_root / f"{user_id}_{lv}_detail.html").is_file()
    ]
    list_name = f"{user_id}_list.html"
    if (source_root / list_name).is_file():
        relative_paths.append(list_name)
    # 一覧HTMLは過去の放送も参照する。選択分だけ上げるとリンク切れになるので、
    # ローカルにある個別HTMLは全部上げる。
    relative_paths.extend(
        sorted(
            path.name
            for path in source_root.glob(f"{user_id}_lv*_detail.html")
            if path.is_file()
        )
    )
    for asset_dir in ("css", "js"):
        directory = source_root / asset_dir
        if directory.is_dir():
            relative_paths.extend(
                f"{asset_dir}/{path.name}" for path in sorted(directory.iterdir()) if path.is_file()
            )
    if not any(name.endswith("_detail.html") for name in relative_paths):
        return {
            "uploaded": False,
            "reason": "アップロードできる個別HTMLがありません",
            "missing_lvs": missing_lvs,
        }

    python_exe = Path(str(config.archive_upload_python_exe or sys.executable))
    cli_path = Path(str(config.archive_upload_cli_path or ""))
    if not python_exe.is_file():
        return {"uploaded": False, "reason": f"Pythonが見つかりません: {python_exe}"}
    if not cli_path.is_file():
        return {"uploaded": False, "reason": f"upload-targets CLIが見つかりません: {cli_path}"}

    # CLIはパスワード管理API経由でしか認証しない。止まっていたら起動させる。
    if bool(config.archive_upload_auto_start_credentials_api):
        try:
            ensure_credentials_api_running(config, progress_callback=progress_callback)
        except Exception as exc:
            return {"uploaded": False, "reason": f"パスワード管理APIを起動できません: {exc}"}

    target_id = str(config.archive_upload_target_id or "lolipop-main").strip()
    remote_directory = f"niconico/{user_id}/special_user"
    command = [
        str(python_exe),
        str(cli_path),
        "upload",
    ...
    if bool(config.archive_upload_http_verify):
        command.append("--http-verify")

    if progress_callback:
        progress_callback(
            f"{user_id}: {len(relative_paths)}ファイルを {remote_directory} へ公開開始"
        )
    completed = subprocess.run(
        ...
        encoding="utf-8",
        errors="replace",
        timeout=max(30, int(config.archive_upload_timeout_seconds or 900)),
    )
    if completed.returncode != 0:
        detail = (completed.stderr or completed.stdout or "").strip()
        return {
            "uploaded": False,
            "reason": f"アップロード失敗(exit={completed.returncode}): {detail[:400]}",
            "command": command,
        }

    # 実際に送ったのは選択分だけでなく、ディレクトリにある個別HTML全部。
    uploaded_lvs = sorted(
        {
            name[len(user_id) + 1 : -len("_detail.html")]
    ...
    current_time = now_micro()
    with connect() as conn:
        for lv in uploaded_lvs:
            conn.execute(
                """
                INSERT INTO special_user_html_uploads (user_id, lv, uploaded_at, remote_directory)
                VALUES (?, ?, ?, ?)
                ON CONFLICT(user_id, lv) DO UPDATE SET
                    uploaded_at = excluded.uploaded_at,
                    remote_directory = excluded.remote_directory
                """,
                (user_id, lv, current_time, remote_directory),
            )
            # ヒット記録がある放送は従来の列も合わせて更新しておく。
            conn.execute(
                """
                UPDATE special_user_broadcast_hits
                SET html_uploaded_at = ?
                WHERE user_id = ? AND lv = ?
                """,
                (current_time, user_id, lv),
            )
        conn.commit()
    if progress_callback:
        progress_callback(f"{user_id}: {len(uploaded_lvs)}放送を公開しました")
    return {
        "uploaded": True,
        "user_id": user_id,
        "lvs": uploaded_lvs,
...
def save_monitored_broadcaster_details(broadcaster_id: str, values: dict[str, Any]) -> None:
    allowed = {
        ...
        *default_broadcaster_monitor_settings().keys(),
        "html_schedule_enabled",
        "html_schedule_start",
        "html_schedule_end",
        "html_backlog_scan_enabled",
        "html_schedule_max_per_window",
        "summary_engine",
        ...
        "conversation_model",
        "conversation_effort",
        "special_user_summary_model",
        "special_user_summary_effort",
        "ai_analysis_model",
        "ai_analysis_api_key",
        "ai_reaction_model",
        "ai_reaction_api_key",
...
def list_special_users() -> list[dict[str, Any]]:
    with connect() as conn:
        rows = conn.execute(
            """
            SELECT user_id, label, note, enabled, html_generation_enabled, updated_at
            FROM special_users
...
def update_special_user_html_generation_enabled(user_id: str, enabled: bool) -> None:
    with connect() as conn:
        conn.execute(
            "UPDATE special_users SET html_generation_enabled = ?, updated_at = ? WHERE user_id = ?",
            (int(enabled), now(), user_id),
        )
        conn.commit()
...
def load_config() -> Config:
    ...
    config = Config(
        ...
        enable_summary_text=bool(raw.get("enable_summary_text", True)),
        enable_summary_image=bool(raw.get("enable_summary_image", True)),
        enable_ai_conversation=bool(raw.get("enable_ai_conversation", True)),
        enable_ai_music=bool(raw.get("enable_ai_music", True)),
        ...
        suno_music_provider=str(raw.get("suno_music_provider", "api")),
...
def load_shared_env_value(name: str) -> str:
    if not SHARED_ENV_PATH.is_file():
        return ""
    for raw_line in SHARED_ENV_PATH.read_text(encoding="utf-8-sig").splitlines():
        line = raw_line.strip()
...
def save_shared_env_value(name: str, value: str) -> None:
    SHARED_ENV_PATH.parent.mkdir(parents=True, exist_ok=True)
    lines = (
        SHARED_ENV_PATH.read_text(encoding="utf-8-sig").splitlines()
...
def ensure_schema(conn: sqlite3.Connection) -> None:
    ...
    for column, definition in live_stat_columns.items():
        if column not in broadcast_archive_meta_columns:
            conn.execute(
                f"ALTER TABLE broadcast_archive_meta ADD COLUMN {column} {definition}"
            )
    ...
    for name, definition in required.items():
        if name not in columns:
            ...
            if name == "analysis_prompt":
                # 列を足した時点の初期値。以後はユーザー設定として編集できる。
                conn.execute(
                    "UPDATE special_users SET analysis_prompt = ? "
                    "WHERE COALESCE(TRIM(analysis_prompt), '') = ''",
                    (DEFAULT_SPECIAL_USER_ANALYSIS_PROMPT,),
                )
    broadcaster_columns = {
    ...
    monitored_required = {
        ...
        "summary_effort": "TEXT NOT NULL DEFAULT ''",
        "conversation_model": "TEXT NOT NULL DEFAULT ''",
        "conversation_effort": "TEXT NOT NULL DEFAULT ''",
        "special_user_summary_model": "TEXT NOT NULL DEFAULT ''",
        "special_user_summary_effort": "TEXT NOT NULL DEFAULT ''",
    }
...
def record_live_watch_count(conn: sqlite3.Connection, lv: str, watch_count: Any) -> None:
    """放送中に取れた来場者数を broadcast_archive_meta へ残す。

    broadcasts テーブルは TRACKER_BROADCAST_TTL_SECONDS で1時間後に消えるため、
    そこにしか無いと放送履歴に出せなくなる。放送中は増える一方なので最大値を採る。
    """
    lv = str(lv or "").strip()
    count = _int_or_none(watch_count)
    if not lv or count is None or count <= 0:
        return
    conn.execute(
        """
        UPDATE broadcast_archive_meta
        SET watch_count = MAX(COALESCE(watch_count, 0), ?)
        WHERE lv = ?
        """,
        (count, lv),
    )
