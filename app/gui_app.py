...
from PyQt6.QtCore import QAbstractTableModel, QByteArray, QDate, QItemSelectionModel, QModelIndex, QObject, QProcess, QProcessEnvironment, QRunnable, QSize, QThread, QThreadPool, QTime, QTimer, Qt, QUrl, pyqtSignal, pyqtSlot
...
from timeshift_handoff import normalize_local_lvs as normalize_handoff_local_lvs
...
from timeshift_handoff import send_local_targets as send_handoff_local_targets
...
from timeshift_handoff import (
    normalize_special_user_jobs as normalize_handoff_special_user_jobs,
)
from timeshift_handoff import send_special_user_jobs as send_handoff_special_user_jobs
...
def send_special_user_jobs_to_timeshift_gui(jobs: list[dict[str, str]]) -> str:
    """未処理のスペシャルユーザー放送をタイムシフトGUIへ渡す。"""
    normalized = normalize_handoff_special_user_jobs(jobs)
    if not normalized:
        raise ValueError("タイムシフトGUIへ送れる放送がありません")
    if send_handoff_special_user_jobs(normalized, timeout_ms=300):
        return "sent"
    ...
    for job in normalized:
        arguments.extend(["--special-user-job", f"{job['user_id']}:{job['lv']}"])
    started = QProcess.startDetached(str(python_exe), arguments, str(APP_ROOT))
    success = bool(started[0]) if isinstance(started, tuple) else bool(started)
    if not success:
        raise RuntimeError("タイムシフトGUIを起動できません")
...
def send_local_targets_to_processing_gui(
    paths: list[Path | str],
    lvs: list[str],
) -> str:
    """動画とLV指定をまとめてローカル処理GUIへ渡す。

    動画が消えた放送はLVだけで送る。動画側とLV側を別メッセージにすると、
    GUI未起動時に1通目で起動→2通目が接続失敗→二重起動になるため必ず1通で送る。
    """
    normalized_paths = [
        path for path in normalize_handoff_local_files(paths) if Path(path).is_file()
    ]
    normalized_lvs = normalize_handoff_local_lvs(lvs)
    if not normalized_paths and not normalized_lvs:
        raise ValueError("ローカル処理へ送れる動画もLVもありません")
    if send_handoff_local_targets(normalized_paths, normalized_lvs, timeout_ms=300):
        return "sent"
    ...
    for path in normalized_paths:
        arguments.extend(["--input-file", path])
    for lv in normalized_lvs:
        arguments.extend(["--input-lv", lv])
    started = QProcess.startDetached(str(python_exe), arguments, str(APP_ROOT))
    success = bool(started[0]) if isinstance(started, tuple) else bool(started)
    if not success:
        raise RuntimeError("ローカル処理GUIを起動できません")
...
def broadcast_duration_text(begin_value: Any, end_value: Any) -> str:
    """開始と終了のunix秒から放送時間を「1時間23分」形式にする。"""
    try:
        begin = int(begin_value or 0)
        end = int(end_value or 0)
    except (TypeError, ValueError):
        return ""
...
def install_table_copy_menu(table: QTableView) -> None:
    ...
    def show_menu(position) -> None:
        ...
        if generated_url.startswith(("http://", "https://")):
            menu.addSeparator()
            # タグ編集は配信者アーカイブHTML専用。タグ機構を持たないページでは出さない。
            if bool(getattr(table, "_tag_edit_enabled", True)):
...
def apply_table_header_state(table: QTableView, state: object) -> None:
    ...
    header = table.horizontalHeader()
    try:
        restored = bool(header.restoreState(QByteArray.fromHex(state.encode("ascii"))))
    except Exception:
        append_app_log(traceback.format_exc(), "DEBUG")
        return
    # 列を増やすと保存済み状態の列数が合わず、見出しと中身がずれたり列が消える。
    # 復元に失敗したか、隠れた列ができたら、保存状態を捨てて既定の並びに戻す。
    if not restored or any(header.isSectionHidden(i) for i in range(header.count())):
        for index in range(header.count()):
            header.setSectionHidden(index, False)
        for logical in range(header.count()):
            current = header.visualIndex(logical)
            if current != logical:
                header.moveSection(current, logical)
...
class SpecialUserHtmlSignals(QObject):
    finished = pyqtSignal(str, object)
    failed = pyqtSignal(str, str)


class SpecialUploadJob(QRunnable):
    """生成済みのスペシャルユーザーHTMLをサーバーへ公開する。"""

    def __init__(self, user_id: str, lvs: list[str]) -> None:
        super().__init__()
        self.user_id = user_id
        self.lvs = lvs
        self.signals = SpecialUserHtmlSignals()

    def run(self) -> None:
        try:
            result = tracker.upload_special_user_html(self.user_id, self.lvs)
            self.signals.finished.emit(self.user_id, result)
        except Exception as exc:
            append_app_log(traceback.format_exc(), "DEBUG")
            self.signals.failed.emit(self.user_id, f"{type(exc).__name__}: {exc}")


class SpecialUserHtmlJob(QRunnable):
    """HTML生成ONのスペシャルユーザーのページを、放送終了後にまとめて作る。

    AI分析でCLIを呼ぶので数分かかることがある。GUIスレッドを止めないため別スレッドで走らせる。
    """

    def __init__(self, lv: str, force_overwrite: bool = False) -> None:
        super().__init__()
        self.lv = lv
    ...
    def run(self) -> None:
        try:
            results = tracker.generate_special_user_html_for_lv(
                self.lv, force_overwrite_existing_html=self.force_overwrite
            )
            self.signals.finished.emit(self.lv, results)
        except Exception as exc:
            append_app_log(traceback.format_exc(), "DEBUG")
            self.signals.failed.emit(self.lv, f"{type(exc).__name__}: {exc}")
...
class FinalizeBacklogScanSignals(QObject):
    finished = pyqtSignal(object)


class FinalizeBacklogScanJob(QRunnable):
    """未処理放送の走査は録画フォルダを舐めるので、UIスレッドから外して回す。"""

    def __init__(self) -> None:
        super().__init__()
        self.signals = FinalizeBacklogScanSignals()

    def run(self) -> None:
        enqueued: list[dict[str, Any]] = []
        try:
            enqueued = tracker.maybe_enqueue_backlog_finalize_items()
        except Exception as exc:
            append_app_log(f"未処理放送の走査に失敗: {type(exc).__name__}: {exc}", "WARN")
            append_app_log(traceback.format_exc(), "DEBUG")
        self.signals.finished.emit(enqueued)
...
class SpecialUsersModel(QAbstractTableModel):
    ...
    html_generation_changed = pyqtSignal(str, bool)
    checkable_columns = ("enabled", "html_generation_enabled")
    columns = [
        ("enabled", "有効"),
    ...
    def data(self, index: QModelIndex, role: int = Qt.ItemDataRole.DisplayRole) -> Any:
        ...
        if key in self.checkable_columns and role == Qt.ItemDataRole.CheckStateRole:
            return Qt.CheckState.Checked if value else Qt.CheckState.Unchecked
        if role not in (Qt.ItemDataRole.DisplayRole, Qt.ItemDataRole.ToolTipRole):
            return None
        if key in self.checkable_columns:
            return ""
        return "" if value is None else str(value)
    ...
    def setData(self, index: QModelIndex, value: Any, role: int = Qt.ItemDataRole.EditRole) -> bool:
        if not index.isValid() or role != Qt.ItemDataRole.CheckStateRole:
            return False
        key = self.columns[index.column()][0]
        if key not in self.checkable_columns:
            return False
        enabled = value == Qt.CheckState.Checked.value
        self._rows[index.row()][key] = int(enabled)
        ...
        if user_id:
            if key == "enabled":
                ...
                self.html_generation_changed.emit(user_id, enabled)
        return True
    ...
    def flags(self, index: QModelIndex) -> Qt.ItemFlag:
        ...
        if self.columns[index.column()][0] in self.checkable_columns:
...
class SpecialUsersTab(QWidget):
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self.model.html_generation_changed.connect(self.on_html_generation_changed)
        ...
        self.table.setAlternatingRowColors(True)
        self.table.verticalHeader().setVisible(False)
        configure_table_header(self.table, [60, 80, 180, 180, 360])
    ...
    def on_html_generation_changed(self, user_id: str, enabled: bool) -> None:
        tracker.update_special_user_html_generation_enabled(user_id, enabled)
        show_status(self, f"スペシャルユーザーHTML生成切替: {user_id} = {int(enabled)}")
...
CODEX_FALLBACK_MODELS = ["gpt-5.6-sol", "gpt-5.6-terra", "gpt-5.6-luna", "gpt-5.5"]
CODEX_FALLBACK_EFFORTS = ["low", "medium", "high", "xhigh", "max"]
# claude --model / --effort が受ける値（フルモデル名も可なのでコンボは編集可のまま）
CLAUDE_CLI_MODELS = ["sonnet", "opus", "haiku", "fable"]
CLAUDE_CLI_EFFORTS = ["low", "medium", "high", "xhigh", "max", "ultracode"]
GROK_FALLBACK_MODELS = ["grok-4.5"]
GROK_FALLBACK_EFFORTS = ["low", "medium", "high"]


def load_codex_model_efforts() -> dict[str, list[str]]:
    """Codex CLIのキャッシュからモデルごとの推論レベルを読む。読めなければ空。"""
    cache_path = Path(
    ...
    try:
        ...
        for row in payload.get("models", []):
            ...
            if not slug or str(row.get("visibility") or "list") == "hide":
                continue
            out[slug] = [
            ...
            ] or list(CODEX_FALLBACK_EFFORTS)
    except Exception as exc:
        append_app_log(f"Codexモデル一覧読込失敗: {type(exc).__name__}: {exc}", "WARN")
    return out


def load_grok_model_efforts() -> dict[str, list[str]]:
    """Grok CLIのキャッシュからモデルごとの推論レベルを読む。Codexとは形式が違う。"""
    cache_path = Path(
    ...
    try:
        ...
        for key, entry in (models.items() if isinstance(models, dict) else []):
            ...
            if bool(info.get("hidden")):
                continue
            slug = str(info.get("model") or info.get("id") or key or "").strip()
            if not slug:
                continue
            if not info.get("supports_reasoning_effort", True):
                out[slug] = []
                continue
            out[slug] = [
                str(level.get("value") or level.get("id") or "").strip()
                for level in info.get("reasoning_efforts", [])
                if isinstance(level, dict)
                and str(level.get("value") or level.get("id") or "").strip()
            ] or list(GROK_FALLBACK_EFFORTS)
    except Exception as exc:
        append_app_log(f"Grokモデル一覧読込失敗: {type(exc).__name__}: {exc}", "WARN")
    return out


def load_codex_model_slugs() -> list[str]:
    return list(load_codex_model_efforts()) or list(CODEX_FALLBACK_MODELS)


def load_grok_model_slugs() -> list[str]:
    return list(load_grok_model_efforts()) or list(GROK_FALLBACK_MODELS)
...
API_EFFORTS = ["minimal", "low", "medium", "high"]


def ai_model_choices_for_engine(engine: str) -> list[str]:
    """担当エンジンで選べるモデル。CLI系は各CLIのキャッシュから引く。"""
    if engine == "codex_exec":
        return load_codex_model_slugs()
    if engine == "claude":
        return list(CLAUDE_CLI_MODELS)
    if engine == "grok":
        return load_grok_model_slugs()
    if engine == "gemini":
        return list(GEMINI_API_MODELS)
    return list(OPENAI_API_MODELS)


def ai_model_belongs_to_other_engine(engine: str, model: str) -> bool:
    """そのモデル名が別エンジン専用と分かる場合だけTrue。

    モデル欄は手入力も許すので、候補に無いというだけでは消さない。
    「担当=ClaudeCode なのに gpt-5」のように取り違えが明らかな時だけ消す。
    """
    model = str(model or "").strip()
    if not model or model in ai_model_choices_for_engine(engine):
        return False
    return any(
        model in ai_model_choices_for_engine(other)
        for other in ("codex_exec", "claude", "grok", "openai", "gemini")
        if other != engine
    )


def ai_effort_choices_for(engine: str, model: str = "") -> list[str]:
    """推論(effort)の候補。CLI系はモデルごとに対応レベルが違う。"""
    model = str(model or "").strip()
    if engine == "claude":
        return list(CLAUDE_CLI_EFFORTS)
    if engine in {"codex_exec", "grok"}:
        table = load_codex_model_efforts() if engine == "codex_exec" else load_grok_model_efforts()
        fallback = CODEX_FALLBACK_EFFORTS if engine == "codex_exec" else GROK_FALLBACK_EFFORTS
        if model in table:
            return list(table[model])
        # モデル未指定(=CLIの既定)や未知の名前は、対応レベルの和集合を出す
        merged: list[str] = []
        for levels in table.values():
            for level in levels:
                if level not in merged:
                    merged.append(level)
        return merged or list(fallback)
    return list(API_EFFORTS)
...
class SpecialUserEditorDialog(QDialog):
    ...
    def __init__(self, user_id: str, parent: QWidget | None = None) -> None:
        ...
        self.analysis_prompt.setMinimumHeight(110)
        self.analysis_prompt.setPlaceholderText(
            "コメント分析の指示文。空なら既定文を使う:\n"
            + tracker.DEFAULT_SPECIAL_USER_ANALYSIS_PROMPT
        )
        self.analysis_system_prompt = QTextEdit()
        self.analysis_system_prompt.setMinimumHeight(80)
        self.analysis_system_prompt.setPlaceholderText(
            "AIの役割指定(system)。空なら付けない。{name} はユーザー名に置き換わる"
        )
        ...
        self.html_generation_enabled = QCheckBox("HTMLを生成する")
        self.html_generation_enabled.setToolTip(
            "放送終了時に、このユーザーの一覧HTMLと個別HTMLを作る"
        )
        self.analysis_enabled = QCheckBox("AI分析を使う")
        self.analysis_enabled.setToolTip(
            "個別HTMLにAI分析を載せる。OFFならコメント表と基本統計だけになる"
        )
        self.html_upload_enabled = QCheckBox("サーバーへアップロードする")
        self.html_upload_enabled.setToolTip(
            "生成したHTMLをサーバーへ公開する。HTML生成がOFFなら何も起きない"
        )
        self.server_settings_button = QPushButton("サーバー設定")
        self.server_settings_button.clicked.connect(self.open_server_settings)
        self.design_editor_button = QPushButton("ページデザインを編集")
        self.design_editor_button.setToolTip(
            "ブラウザで見本ページを開き、背景・粒子・フォントをその場で変えて保存する"
        )
        self.design_editor_button.clicked.connect(self.open_design_editor)
        self.link_design_editor_button = QPushButton("リンクページデザインを編集")
        self.link_design_editor_button.setToolTip(
            "実際のリンク一覧HTMLと同じ構造の見本を開き、個別ページと同じ編集UIで変更する"
        )
        self.link_design_editor_button.clicked.connect(self.open_link_design_editor)
        ...
        basic_layout.addWidget(self.analysis_enabled)
        basic_layout.addWidget(self.html_upload_enabled)
        basic_layout.addWidget(self.server_settings_button)
        basic_layout.addWidget(self.design_editor_button)
        basic_layout.addWidget(self.link_design_editor_button)
        ...
        analysis_layout.addWidget(QLabel("分析プロンプト"))
        analysis_layout.addWidget(self.analysis_prompt)
        analysis_layout.addWidget(QLabel("AIの役割指定(system)"))
        analysis_layout.addWidget(self.analysis_system_prompt)
        prompt_variables = QLabel(
            "使える変数: {name}=表示名 / {user_id}=ユーザーID / {title}=放送タイトル /"
            " {lv}=LV / {broadcaster}=配信者名 / {start_time}=放送開始日時 /"
            " {comment_count}=そのユーザーのコメント数"
        )
        prompt_variables.setWordWrap(True)
        prompt_variables.setStyleSheet("color: #9b9ba0; font-size: 11px")
        analysis_layout.addWidget(prompt_variables)
    ...
    def load(self) -> None:
        with tracker.connect() as conn:
            user = conn.execute(
                ...
                SELECT user_id, label, analysis_model, analysis_api_key,
                       ...
                       analysis_prompt, analysis_system_prompt, analysis_enabled,
        ...
        if user:
            ...
            # 未設定なら既定文をそのまま入れておく。何が使われるか画面で分かるようにする。
            self.analysis_prompt.setPlainText(
                str(user["analysis_prompt"] or "").strip()
                or tracker.DEFAULT_SPECIAL_USER_ANALYSIS_PROMPT
            )
            self.analysis_system_prompt.setPlainText(user["analysis_system_prompt"] or "")
            self.analysis_enabled.setChecked(bool(user["analysis_enabled"]))
            self.html_generation_enabled.setChecked(bool(user["html_generation_enabled"]))
            ...
            self.html_upload_enabled.setChecked(bool(user["html_upload_enabled"]))
    ...
    def open_design_editor(self) -> None:
        """見本ページをブラウザで開く。保存はページ内のボタンから。"""
        try:
            import special_user_design

            url = special_user_design.start_editor_server(
                self.user_id, self.label_input.text().strip() or self.user_id
            )
        except Exception as exc:
            append_app_log(traceback.format_exc(), "DEBUG")
            QMessageBox.critical(self, "ページデザイン編集", f"{type(exc).__name__}: {exc}")
            return
        QDesktopServices.openUrl(QUrl(url))
        show_status(self, f"ページデザイン編集を開きました: {url}")

    def open_link_design_editor(self) -> None:
        """Step11が生成したリンク一覧ダミーを既存の共通編集UIで開く。"""
        try:
            import special_user_design

            url = special_user_design.start_link_editor_server(
                self.user_id, self.label_input.text().strip() or self.user_id
            )
        except Exception as exc:
            append_app_log(traceback.format_exc(), "DEBUG")
            ...
            return
        QDesktopServices.openUrl(QUrl(url))
        show_status(self, f"リンクページデザイン編集を開きました: {url}")
        ...
        with tracker.connect() as conn:
            conn.execute(
                ...
                (
                    ...
                    self.analysis_prompt.toPlainText().strip(),
                    self.analysis_system_prompt.toPlainText().strip(),
                    int(self.analysis_enabled.isChecked()),
                    int(self.html_generation_enabled.isChecked()),
                    ...
                    int(self.html_upload_enabled.isChecked()),
    ...
    def refresh_ai_model_combo(self, engine_combo: QComboBox, model_combo: QComboBox) -> None:
        ...
        models: list[str]
        if engine == "codex_exec":
            models = load_codex_model_slugs()
        elif engine == "claude":
            models = list(CLAUDE_CLI_MODELS)
        elif engine == "grok":
            models = load_grok_model_slugs()
        elif engine == "gemini":
...
"""
        ...
        self.start_special_user_html_job()
        try:
    ...
    def start_special_user_html_job(self) -> None:
        """HTML生成ONのスペシャルユーザーのページ生成を別スレッドで開始する。"""
        job = SpecialUserHtmlJob(self.lv)
        job.signals.finished.connect(self.on_special_user_html_finished)
        job.signals.failed.connect(self.on_special_user_html_failed)
        QThreadPool.globalInstance().start(job)

    def on_special_user_html_finished(self, lv: str, results: object) -> None:
        rows = list(results) if isinstance(results, list) else []
        if not rows:
            return
        ok = sum(1 for row in rows if row.get("status") == "generated")
        failed = len(rows) - ok
        show_status(self, f"スペシャルユーザーHTML生成: {lv} / 成功 {ok} / 失敗 {failed}")
        for row in rows:
            if row.get("status") == "generated":
                append_app_log(
                    f"スペシャルユーザーHTML生成: {lv} / {row.get('user_id')} / "
                    f"{row.get('comments')}コメント / {row.get('output_dir')}",
                    "INFO",
                )
            else:
                append_app_log(
                    f"スペシャルユーザーHTML生成失敗: {lv} / {row.get('user_id')} / {row.get('error')}",
                    "WARN",
                )

    def on_special_user_html_failed(self, lv: str, error: str) -> None:
        show_status(self, f"スペシャルユーザーHTML生成失敗: {lv} / {error}", "ERROR")
...
class MonitoredBroadcasterEditorDialog(QDialog):
    def __init__(self, broadcaster: dict[str, Any], parent: QWidget | None = None) -> None:
        ...
        self.html_schedule_enabled = QCheckBox("時間帯を決めて処理する")
        self.html_schedule_enabled.setChecked(bool(broadcaster.get("html_schedule_enabled", 0)))
        self.html_schedule_enabled.setToolTip(
            "OFFなら放送終了と同時にHTMLを作る。ONにすると、決めた時刻まで待って古い順に処理する"
        )
        self.html_schedule_start = QTimeEdit()
        self.html_schedule_start.setDisplayFormat("HH:mm")
        self.html_schedule_start.setTime(
            self.schedule_time_value(broadcaster.get("html_schedule_start"), QTime(5, 0))
        )
        self.html_schedule_no_end = QCheckBox("終了時刻を決めない（溜まった分が無くなるまで処理する）")
        self.html_schedule_end = QTimeEdit()
        self.html_schedule_end.setDisplayFormat("HH:mm")
        end_time = self.schedule_time_value(broadcaster.get("html_schedule_end"), None)
        self.html_schedule_no_end.setChecked(end_time is None)
        self.html_schedule_end.setTime(end_time or QTime(9, 0))
        self.html_backlog_scan_enabled = QCheckBox("未処理の過去放送も処理する")
        self.html_backlog_scan_enabled.setChecked(
            bool(broadcaster.get("html_backlog_scan_enabled", 1))
        )
        self.html_backlog_scan_enabled.setToolTip(
            "HTMLが無く、録画素材が残っている過去放送を、枠が開いた時に古い順で処理する"
        )
        self.html_schedule_max_per_window = NoWheelSpinBox()
        self.html_schedule_max_per_window.setRange(0, 999)
        self.html_schedule_max_per_window.setValue(
            int(broadcaster.get("html_schedule_max_per_window") or 0)
        )
        self.html_schedule_max_per_window.setToolTip("0で無制限")
        self.html_generation_enabled.toggled.connect(self.update_html_schedule_fields)
        self.html_schedule_enabled.toggled.connect(self.update_html_schedule_fields)
        self.html_schedule_no_end.toggled.connect(self.update_html_schedule_fields)
        self.html_backlog_scan_enabled.toggled.connect(self.update_html_schedule_fields)
        self.video_archive_enabled = QCheckBox("処理後に録画動画をHTMLフォルダ配下へ集約する")
        self.video_archive_enabled.setChecked(
            bool(broadcaster.get("video_archive_enabled", 1))
        )
        ...
        def make_model_field(value):
            combo = NoWheelComboBox()
            combo.setEditable(True)
            combo.lineEdit().setPlaceholderText("空=全体設定")
            combo.setCurrentText(str(value or ""))
            return combo

        def make_effort_combo(value):
            combo = NoWheelComboBox()
            combo.addItem("既定（推論なし/モデル依存）", "")
            # 保存済みの値をいったん載せておく。直後のrefreshで
            # そのエンジン/モデルが対応していれば残り、していなければ既定へ戻る。
            saved = str(value or "").strip()
            if saved:
                combo.addItem(saved, saved)
            self.set_combo_value(combo, saved)
            return combo

        self.summary_model = make_model_field(broadcaster.get("summary_model"))
        self.summary_effort = make_effort_combo(broadcaster.get("summary_effort"))
        self.conversation_model = make_model_field(broadcaster.get("conversation_model"))
        self.conversation_effort = make_effort_combo(broadcaster.get("conversation_effort"))
        self.special_user_summary_model = make_model_field(broadcaster.get("special_user_summary_model"))
        self.special_user_summary_effort = make_effort_combo(broadcaster.get("special_user_summary_effort"))
        # 担当エンジンに合わせてモデル候補を差し替える（Codex/ClaudeCode/Grokもここで選べる）。
        # 推論(effort)はエンジンだけでなくモデルにも依存するので、モデル変更にも追従させる。
        for engine_combo, model_combo, effort_combo in (
            (self.summary_engine, self.summary_model, self.summary_effort),
            (self.ai_conversation_engine, self.conversation_model, self.conversation_effort),
            (self.special_user_summary_engine, self.special_user_summary_model, self.special_user_summary_effort),
        ):
            self.refresh_ai_model_combo(engine_combo, model_combo)
            self.refresh_ai_effort_combo(engine_combo, model_combo, effort_combo)
            engine_combo.currentIndexChanged.connect(
                lambda _index=0, e=engine_combo, m=model_combo, f=effort_combo: (
                    self.refresh_ai_model_combo(e, m),
                    self.refresh_ai_effort_combo(e, m, f),
                )
            )
            model_combo.currentTextChanged.connect(
                lambda _text="", e=engine_combo, m=model_combo, f=effort_combo:
                    self.refresh_ai_effort_combo(e, m, f)
            )
        ...
        basic_layout.addWidget(self.enabled)
        basic_layout.addWidget(self.html_generation_enabled)
        basic_layout.addLayout(self.build_html_schedule_layout())
        basic_layout.addWidget(self.video_archive_enabled)
        ...
        self.add_field(ai_layout, "　└ 要約モデル", self.summary_model)
        self.add_field(ai_layout, "　└ 要約の推論(effort)", self.summary_effort)
        self.add_field(ai_layout, "ニニココ会話担当", self.ai_conversation_engine)
        self.add_field(ai_layout, "　└ 会話モデル", self.conversation_model)
        self.add_field(ai_layout, "　└ 会話の推論(effort)", self.conversation_effort)
        self.add_field(ai_layout, "スペシャルユーザーまとめ担当", self.special_user_summary_engine)
        self.add_field(ai_layout, "　└ スペシャルまとめモデル", self.special_user_summary_model)
        self.add_field(ai_layout, "　└ スペシャルまとめの推論(effort)", self.special_user_summary_effort)
    ...
    def _single_char_entries(self) -> list[str]:
        """認識支援文字/正式タグ/別名に1文字の値があれば、その説明一覧を返す。

        1文字は本文全体へ誤マッチし、人物ハイライトや認識を壊すため禁止する。
        """
        problems: list[str] = []
        for row in range(self.archive_tags_table.rowCount()):
            recognition_item = self.archive_tags_table.item(row, 0)
            canonical_item = self.archive_tags_table.item(row, 1)
            aliases_item = self.archive_tags_table.item(row, 2)
            recognition = recognition_item.text().strip() if recognition_item else ""
            canonical = canonical_item.text().strip() if canonical_item else ""
            if len(recognition) == 1:
                problems.append(f"認識支援文字「{recognition}」")
            if len(canonical) == 1:
                problems.append(f"正式タグ「{canonical}」")
            for value in str(aliases_item.text() if aliases_item else "").split(","):
                alias = value.strip()
                if len(alias) == 1:
                    problems.append(f"別名「{alias}」")
        return list(dict.fromkeys(problems))

    def accept(self) -> None:
        problems = self._single_char_entries()
        if problems:
            QMessageBox.warning(
                self,
                "監視配信者編集",
                "1文字は登録できません（本文全体に誤マッチして事故の原因になります）:\n"
                + "\n".join(problems),
            )
            return
        super().accept()
    ...
    def refresh_ai_effort_combo(
        self, engine_combo: QComboBox, model_combo: QComboBox, effort_combo: QComboBox
    ) -> None:
        current = str(effort_combo.currentData() or "")
        engine = str(engine_combo.currentData() or "")
        choices = ai_effort_choices_for(engine, model_combo.currentText())
        effort_combo.blockSignals(True)
        effort_combo.clear()
        effort_combo.addItem("既定（推論なし/モデル依存）", "")
        for effort in choices:
            effort_combo.addItem(effort, effort)
        # 対応しないレベルが残るとCLIに渡って落ちるので既定へ戻す
        self.set_combo_value(effort_combo, current if current in choices else "")
        effort_combo.blockSignals(False)

    def refresh_ai_model_combo(self, engine_combo: QComboBox, model_combo: QComboBox) -> None:
        current = model_combo.currentText().strip()
        engine = str(engine_combo.currentData() or "")
        choices = [""] + ai_model_choices_for_engine(engine)  # 空=全体設定
        model_combo.blockSignals(True)
        model_combo.clear()
        model_combo.addItems(choices)
        # 担当を切り替えて別エンジンのモデル名が残ると誤ったCLIへ渡るので、その時だけ空へ戻す
        model_combo.setCurrentText(
            "" if ai_model_belongs_to_other_engine(engine, current) else current
        )
        model_combo.blockSignals(False)
    ...
    @staticmethod
    def schedule_time_value(value: Any, fallback: QTime | None) -> QTime | None:
        time_value = QTime.fromString(str(value or "").strip(), "HH:mm")
        if not time_value.isValid():
            time_value = QTime.fromString(str(value or "").strip(), "H:mm")
        return time_value if time_value.isValid() else fallback

    def build_html_schedule_layout(self) -> QVBoxLayout:
        layout = QVBoxLayout()
        layout.setContentsMargins(20, 0, 0, 0)
        layout.addWidget(self.html_schedule_enabled)
        start_row = QHBoxLayout()
        start_row.setContentsMargins(20, 0, 0, 0)
        start_row.addWidget(QLabel("開始"))
        start_row.addWidget(self.html_schedule_start)
        start_row.addStretch(1)
        layout.addLayout(start_row)
        no_end_row = QHBoxLayout()
        no_end_row.setContentsMargins(20, 0, 0, 0)
        no_end_row.addWidget(self.html_schedule_no_end)
        no_end_row.addStretch(1)
        layout.addLayout(no_end_row)
        end_row = QHBoxLayout()
        end_row.setContentsMargins(20, 0, 0, 0)
        end_row.addWidget(QLabel("終了"))
        end_row.addWidget(self.html_schedule_end)
        end_row.addStretch(1)
        layout.addLayout(end_row)
        backlog_row = QHBoxLayout()
        backlog_row.setContentsMargins(20, 0, 0, 0)
        backlog_row.addWidget(self.html_backlog_scan_enabled)
        backlog_row.addWidget(QLabel("1回の枠での最大件数"))
        backlog_row.addWidget(self.html_schedule_max_per_window)
        backlog_row.addStretch(1)
        layout.addLayout(backlog_row)
        self.update_html_schedule_fields()
        return layout

    def update_html_schedule_fields(self) -> None:
        generates_html = self.html_generation_enabled.isChecked()
        self.html_schedule_enabled.setEnabled(generates_html)
        scheduled = generates_html and self.html_schedule_enabled.isChecked()
        self.html_schedule_start.setEnabled(scheduled)
        self.html_schedule_no_end.setEnabled(scheduled)
        self.html_schedule_end.setEnabled(scheduled and not self.html_schedule_no_end.isChecked())
        self.html_backlog_scan_enabled.setEnabled(scheduled)
        self.html_schedule_max_per_window.setEnabled(
            scheduled and self.html_backlog_scan_enabled.isChecked()
        )
    ...
    def values(self) -> dict[str, Any]:
        ...
        return {
            ...
            "html_schedule_enabled": int(self.html_schedule_enabled.isChecked()),
            "html_schedule_start": self.html_schedule_start.time().toString("HH:mm"),
            "html_schedule_end": (
                "" if self.html_schedule_no_end.isChecked()
                else self.html_schedule_end.time().toString("HH:mm")
            ),
            "html_backlog_scan_enabled": int(self.html_backlog_scan_enabled.isChecked()),
            "html_schedule_max_per_window": int(self.html_schedule_max_per_window.value()),
            "video_archive_enabled": int(self.video_archive_enabled.isChecked()),
            ...
            "summary_effort": str(self.summary_effort.currentData() or ""),
            "conversation_model": self.conversation_model.currentText().strip(),
            "conversation_effort": str(self.conversation_effort.currentData() or ""),
            "special_user_summary_model": self.special_user_summary_model.currentText().strip(),
            "special_user_summary_effort": str(self.special_user_summary_effort.currentData() or ""),
...
class BroadcasterMonitorTab(QWidget):
    ...
    def reload(self) -> None:
        ...
        for row in broadcasters:
            ...
            row["html_schedule_label"] = tracker.format_finalize_schedule_label(row)
...
class CommentStreamManager(QObject):
    ...
    def stop(self, lv: str) -> None:
        ...
        if isinstance(process, QProcess) and process.state() != QProcess.ProcessState.NotRunning:
            process.terminate()
            QTimer.singleShot(2000, lambda target_lv=lv: self._kill_if_running(target_lv))
    ...
    def _kill_if_running(self, lv: str) -> None:
        stream = self.streams.get(lv)
        if not stream:
            append_app_log(f"NDGR child kill保険スキップ: {lv} / streamなし(終了済み)", "DEBUG")
            return
        process = stream.get("process")
        if not isinstance(process, QProcess):
            return
        try:
            ...
            append_app_log(f"NDGR child kill保険スキップ: {lv} / QProcess破棄済み", "DEBUG")
            return
        append_app_log(
...
class StorageMigrationWorker(QThread):
    """保存先の移行を別スレッドで回す。GUIは進捗を受けるだけ。"""

    progressed = pyqtSignal(object)
    finished_with_result = pyqtSignal(object)
    failed = pyqtSignal(str)

    def __init__(self, plan: object, parent: QObject | None = None) -> None:
        super().__init__(parent)
        self.plan = plan
    ...
    def run(self) -> None:  # noqa: D102 - QThread側の入口
        try:
            result = storage_migrate.run(
                self.plan,
...
class StorageMigrationDialog(QDialog):
    ...
    移行が全部成功したときだけ commit を呼ぶ。途中で止めた場合は設定を書き換えない。
    移行先に同名同サイズが既にある分は飛ばすので、開き直せば続きから再開できる。
    """
    ...
    def __init__(
        ...
        self._commit = commit
        self.plan: storage_migrate.MigrationPlan | None = None
        self.worker: StorageMigrationWorker | None = None
        self.committed = False

        layout = QVBoxLayout(self)
        layout.addWidget(QLabel(f"移行元: {self.source_root}"))
        layout.addWidget(QLabel(f"移行先: {self.destination_root}"))

        self.summary = QLabel("「移行内容を調べる」を押すと、移す対象を数えます。")
        ...
        self.scan_button = QPushButton("移行内容を調べる")
        self.scan_button.clicked.connect(self.scan)
        self.start_button = QPushButton("移行する")
        self.start_button.setEnabled(False)
    ...
    def scan(self) -> None:
        ...
        self.start_button.setEnabled(False)
        try:
            plan = storage_migrate.build_plan(self.source_root, self.destination_root)
            storage_migrate.check_free_space(plan)
        except ValueError as exc:
            self.plan = None
            self.summary.setText(f"移行できません: {exc}")
            return
        except Exception as exc:
            self.plan = None
            append_app_log(traceback.format_exc(), "ERROR")
            self.summary.setText(f"調査に失敗しました: {exc}")
            return

        self.plan = plan
        move_kind = "同じドライブなので移動のみ（高速）" if plan.same_volume else "別ドライブなのでコピーしてから元を消す"
        lines = [
            f"移す: {plan.file_count} 件 / {storage_paths.format_bytes(plan.total_bytes)}",
            f"移行済みで飛ばす: {len(plan.skipped)} 件",
            move_kind,
        ]
        if plan.conflicts:
            lines.append(f"⚠ 移行先に同名で中身が違うファイルが {len(plan.conflicts)} 件あります。上書きせず残します。")
        self.summary.setText("\n".join(lines))

        for item in plan.items[:2000]:
            self.detail.addItem(
                f"{item.source}  →  {item.destination}  ({storage_paths.format_bytes(item.size)})"
            )
        if plan.file_count > 2000:
            self.detail.addItem(f"... 他 {plan.file_count - 2000} 件")
        for item in plan.conflicts:
            self.detail.addItem(f"[競合・移しません] {item.source}")

        self.progress.setValue(0)
        self.start_button.setEnabled(plan.file_count > 0)
        if plan.file_count == 0:
            self.summary.setText(self.summary.text() + "\n移すものはありません。")

    def start(self) -> None:
        if self.plan is None or self.plan.file_count == 0:
            return
        answer = QMessageBox.question(
            self,
            "移行の確認",
            (
                f"{self.plan.file_count} 件 / {storage_paths.format_bytes(self.plan.total_bytes)} を移します。\n\n"
                f"移行元: {self.source_root}\n移行先: {self.destination_root}\n\n"
                "録画と放送終了後フローを止めてから実行してください。\n"
                "処理中のファイルがあると、その分だけ移行に失敗します。\n\n"
                "実行しますか。"
            ),
        ...
        self.worker = StorageMigrationWorker(self.plan, self)
    ...
    def request_cancel(self) -> None:
        if self.worker is not None:
            ...
            self.cancel_button.setEnabled(False)
            self.current_label.setText("中断しています。処理中のファイルが終わるまで待ちます。")

    def on_progress(self, progress: object) -> None:
        total = max(int(getattr(progress, "total_bytes", 0)), 1)
        done = int(getattr(progress, "done_bytes", 0))
        self.progress.setValue(min(int(done * 100 / total), 100))
        self.current_label.setText(
            f"{getattr(progress, 'done_files', 0)}/{getattr(progress, 'total_files', 0)} 件  "
            f"{storage_paths.format_bytes(done)} / {storage_paths.format_bytes(total)}\n"
            f"{getattr(progress, 'current', '')}"
        )

    def on_failed(self, message: str) -> None:
        self.restore_buttons()
        self.summary.setText(f"移行に失敗しました: {message}")
        show_status(self, f"保存先の移行に失敗しました: {message}", "ERROR")

    def on_finished(self, result: object) -> None:
        self.restore_buttons()
        moved = list(getattr(result, "moved", []))
        failed = list(getattr(result, "failed", []))
        skipped = list(getattr(result, "skipped", []))
        cancelled = bool(getattr(result, "cancelled", False))
        moved_bytes = int(getattr(result, "moved_bytes", 0))

        self.detail.clear()
        for path, error in failed:
            self.detail.addItem(f"[失敗] {path}  :  {error}")

        lines = [
            f"移した: {len(moved)} 件 / {storage_paths.format_bytes(moved_bytes)}",
            f"飛ばした: {len(skipped)} 件",
            f"失敗: {len(failed)} 件",
        ]
        if cancelled:
            lines.append("中断しました。設定は変更していません。もう一度開けば続きから再開できます。")
        elif failed:
            lines.append("失敗が残っているので設定は変更していません。原因を解消して開き直してください。")
        self.summary.setText("\n".join(lines))
        append_app_log(
            f"保存先移行: 移動{len(moved)}件 / 失敗{len(failed)}件 / 中断{cancelled} / {self.source_root} → {self.destination_root}",
            "INFO",
        )
        ...
        if cancelled or failed:
            return
        try:
            self._commit(self.destination_root)
        except Exception as exc:
            append_app_log(traceback.format_exc(), "ERROR")
        ...
        QMessageBox.information(
            self,
            "移行完了",
            f"移行と保存先の切り替えが終わりました。\n新しい保存先: {self.destination_root}",
        )
    ...
    def reject(self) -> None:  # noqa: D102 - 実行中は閉じさせない
        if self.worker is not None and self.worker.isRunning():
            QMessageBox.warning(self, "移行中", "移行中は閉じられません。中断してから閉じてください。")
            return
        super().reject()
...
class SettingsTab(QWidget):
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self.suno_auth_process: QProcess | None = None
        ...
        self.target_root_path = QLineEdit()
        self.target_root_path.setPlaceholderText(str(tracker.DEFAULT_TARGET_ROOT))
        self.target_root_path.setToolTip(
            "HTML・音声・画像・退避した元動画の置き場所。配下に platform/niconico/<配信者ID>/broadcast/ が作られます"
        )
        self.target_root_browse_button = QPushButton("参照")
        self.target_root_browse_button.clicked.connect(self.browse_target_root)
        self.target_root_migrate_button = QPushButton("移行して切り替え")
        self.target_root_migrate_button.setToolTip(
            "現在の保存先の中身を指定先へ移してから、保存先を切り替えます"
        )
        self.target_root_migrate_button.clicked.connect(self.migrate_target_root)
        self.target_root_status = QLabel("")
        self.target_root_status.setWordWrap(True)
        self.recording_root_path = QLineEdit()
        self.recording_root_path.setPlaceholderText(str(tracker.DEFAULT_SLNICO_RECORDING_ROOT))
        self.recording_root_path.setToolTip(
            "SlNicoLiveRecが録画ファイルを書き出す場所。SlNicoLiveRec_config.json の StorageLocation を書き換えます"
        )
        self.recording_root_browse_button = QPushButton("参照")
        self.recording_root_browse_button.clicked.connect(self.browse_recording_root)
        self.recording_root_migrate_button = QPushButton("移行して切り替え")
        self.recording_root_migrate_button.setToolTip(
            "録画済みファイルを指定先へ移してから、録画アプリの保存先を切り替えます"
        )
        self.recording_root_migrate_button.clicked.connect(self.migrate_recording_root)
        self.recording_root_status = QLabel("")
        self.recording_root_status.setWordWrap(True)
        self.storage_measure_button = QPushButton("使用量を計算")
        self.storage_measure_button.setToolTip("保存先を実際に走査して使用量を出します。件数が多いと時間がかかります")
        self.storage_measure_button.clicked.connect(self.measure_storage_usage)
        ...
        self.twocaptcha_api_key.setPlaceholderText("2Captcha API Key")
        self.suno_music_provider = NoWheelComboBox()
        self.suno_music_provider.addItem("Suno API", "api")
        self.suno_music_provider.addItem("Sunoサブスク", "subscription")
        self.suno_music_provider.currentIndexChanged.connect(self.update_suno_provider_fields)
        ...
        self.open_suno_auth_browser_button = QPushButton("サブスク認証Chromeを開く")
        self.open_suno_auth_browser_button.clicked.connect(self.open_suno_auth_browser)
        self.import_suno_auth_button = QPushButton("ログイン後に認証を取り込む")
        self.import_suno_auth_button.clicked.connect(self.import_suno_auth)
        self.suno_auth_status = QLabel("サブスク利用時はSunoログインと2Captcha APIキーが必要")
        self.suno_auth_status.setWordWrap(True)
        ...
        # モデル/推論は担当エンジンで選べる値が違うので、候補を差し替える編集可コンボにする
        self.ai_api_model = NoWheelComboBox()
        self.ai_api_model.setEditable(True)
        self.ai_api_model.lineEdit().setPlaceholderText("例: gpt-4o / gemini-2.5-flash")
        self.codex_exec_command = QLineEdit()
        self.codex_exec_command.setPlaceholderText("codex / claude / grok")
        self.codex_exec_model = NoWheelComboBox()
        self.codex_exec_model.setEditable(True)
        self.codex_exec_model.lineEdit().setPlaceholderText("空=CLIの既定モデル")
        self.codex_exec_effort = NoWheelComboBox()
        self.codex_exec_effort.setEditable(True)
        self.codex_exec_effort.lineEdit().setPlaceholderText("空=CLIの既定")
        self.codex_exec_model.currentTextChanged.connect(
            lambda _text="": self.refresh_ai_text_effort_choices()
        )
        ...
        music_box = QGroupBox("Step06 AI曲生成 / Suno")
        music_layout = QVBoxLayout(music_box)
        music_layout.addWidget(QLabel("生成方法"))
        music_layout.addWidget(self.suno_music_provider)
        ...
        suno_auth_row = QWidget()
        suno_auth_row_layout = QHBoxLayout(suno_auth_row)
        suno_auth_row_layout.setContentsMargins(0, 0, 0, 0)
        suno_auth_row_layout.addWidget(self.open_suno_auth_browser_button)
        suno_auth_row_layout.addWidget(self.import_suno_auth_button)
        music_layout.addWidget(suno_auth_row)
        music_layout.addWidget(self.suno_auth_status)
        ...
        credentials_layout.addRow("2Captcha APIキー", self.twocaptcha_api_key)
        ...
        storage_box = QGroupBox("保存先")
        storage_layout = QVBoxLayout(storage_box)
        storage_layout.addWidget(QLabel("生成物の保存先（HTML・音声・画像・退避した元動画）"))
        target_root_row = QWidget()
        target_root_row_layout = QHBoxLayout(target_root_row)
        target_root_row_layout.setContentsMargins(0, 0, 0, 0)
        target_root_row_layout.addWidget(self.target_root_path, 1)
        target_root_row_layout.addWidget(self.target_root_browse_button)
        target_root_row_layout.addWidget(self.target_root_migrate_button)
        storage_layout.addWidget(target_root_row)
        storage_layout.addWidget(self.target_root_status)
        storage_layout.addWidget(QLabel("録画の保存先（SlNicoLiveRecの書き出し先）"))
        recording_root_row = QWidget()
        recording_root_row_layout = QHBoxLayout(recording_root_row)
        recording_root_row_layout.setContentsMargins(0, 0, 0, 0)
        recording_root_row_layout.addWidget(self.recording_root_path, 1)
        recording_root_row_layout.addWidget(self.recording_root_browse_button)
        recording_root_row_layout.addWidget(self.recording_root_migrate_button)
        storage_layout.addWidget(recording_root_row)
        storage_layout.addWidget(self.recording_root_status)
        storage_measure_row = QWidget()
        storage_measure_layout = QHBoxLayout(storage_measure_row)
        storage_measure_layout.setContentsMargins(0, 0, 0, 0)
        storage_measure_layout.addWidget(self.storage_measure_button)
        storage_measure_layout.addStretch(1)
        storage_layout.addWidget(storage_measure_row)
        storage_layout.addWidget(
            QLabel(
                "パスだけ変えて保存すると、これまでの生成物は元の場所に残ります。"
                "既存ぶんも動かすときは「移行して切り替え」を使ってください。"
            )
        )
    ...
    def load_config_settings(self) -> None:
        try:
            ...
            self.twocaptcha_api_key.setText(tracker.load_shared_env_value("TWOCAPTCHA_API_KEY"))
            self.set_combo_value(self.suno_music_provider, config.suno_music_provider)
            ...
            self.update_suno_provider_fields()
            ...
            self.ai_api_model.setCurrentText(config.ai_text_model)
            self.codex_exec_command.setText(config.codex_exec_command)
            self.codex_exec_model.setCurrentText(config.codex_exec_model)
            self.codex_exec_effort.setCurrentText(config.codex_exec_effort)
    ...
    def refresh_storage_paths(self, *, measure: bool = False) -> None:
        """保存先の欄と、ドライブの空き・使用量の表示を作り直す。"""
        try:
            target_root = storage_paths.read_target_root()
            recording_root = storage_paths.read_recording_root()
        except Exception as exc:
            append_app_log(traceback.format_exc(), "ERROR")
            self.target_root_status.setText(f"保存先を読めません: {exc}")
            return
        self.target_root_path.setText(str(target_root))
        self.recording_root_path.setText(str(recording_root))
        self.target_root_status.setText(self.describe_storage(target_root, measure=measure))
        self.recording_root_status.setText(self.describe_storage(recording_root, measure=measure))

    def describe_storage(self, path: Path, *, measure: bool) -> str:
        try:
            status = storage_paths.describe_root(path, measure=measure)
        except Exception:
            append_app_log(traceback.format_exc(), "DEBUG")
            return f"状態を取得できません: {path}"
        parts = [
            f"ドライブ {status.drive or '不明'}",
            f"空き {storage_paths.format_bytes(status.free_bytes)}"
            f" / 全体 {storage_paths.format_bytes(status.total_bytes)}",
        ]
        if not status.exists:
            parts.append("※このフォルダーはまだありません（保存時に作られます）")
        if measure and status.exists:
            parts.append(f"使用量 {storage_paths.format_bytes(status.used_bytes)}")
        return "　".join(parts)

    def measure_storage_usage(self) -> None:
        self.storage_measure_button.setEnabled(False)
        self.storage_measure_button.setText("計算中…")
        QApplication.processEvents()
        try:
            self.refresh_storage_paths(measure=True)
        finally:
            self.storage_measure_button.setEnabled(True)
            self.storage_measure_button.setText("使用量を計算")

    def save_recording_root(self) -> bool:
        """録画の保存先が変わっていれば SlNicoLiveRec_config.json へ書く。"""
        value = self.recording_root_path.text().strip()
        if not value:
            self.status.setText("保存できない: 録画の保存先が空")
            return False
        try:
            if Path(value) == storage_paths.read_recording_root():
                return True
            storage_paths.write_recording_root(value)
        except Exception as exc:
            append_app_log(traceback.format_exc(), "ERROR")
            self.status.setText(f"録画の保存先を保存できない: {exc}")
            return False
        return True

    def browse_target_root(self) -> None:
        current = self.target_root_path.text().strip() or str(tracker.DEFAULT_TARGET_ROOT)
        path = QFileDialog.getExistingDirectory(self, "生成物の保存先を選ぶ", current)
        if path:
            self.target_root_path.setText(str(Path(path)))
            self.target_root_status.setText(self.describe_storage(Path(path), measure=False))

    def browse_recording_root(self) -> None:
        current = self.recording_root_path.text().strip() or str(tracker.DEFAULT_SLNICO_RECORDING_ROOT)
        path = QFileDialog.getExistingDirectory(self, "録画の保存先を選ぶ", current)
        if path:
            self.recording_root_path.setText(str(Path(path)))
            self.recording_root_status.setText(self.describe_storage(Path(path), measure=False))

    def migrate_target_root(self) -> None:
        destination = self.target_root_path.text().strip()
        if not destination:
            self.status.setText("移行できない: 生成物の保存先が空")
            return
        self.open_migration_dialog(
            "生成物の保存先を移行",
            storage_paths.read_target_root(),
            Path(destination),
            storage_paths.write_target_root,
        )

    def migrate_recording_root(self) -> None:
        destination = self.recording_root_path.text().strip()
        if not destination:
            self.status.setText("移行できない: 録画の保存先が空")
            return
        try:
            config_path = storage_paths.slnico_config_path()
        except Exception as exc:
            self.status.setText(f"移行できない: {exc}")
            return
        if not config_path.is_file():
            self.status.setText(
                f"移行できない: SlNicoLiveRecの設定が見つからない {config_path}"
            )
            return
        self.open_migration_dialog(
            "録画の保存先を移行",
            storage_paths.read_recording_root(),
            Path(destination),
            storage_paths.write_recording_root,
        )

    def open_migration_dialog(
        self,
        title: str,
        source: Path,
        destination: Path,
        ...
        try:
            dialog = StorageMigrationDialog(title, source, destination, commit, self)
        except Exception as exc:
            append_app_log(traceback.format_exc(), "ERROR")
            self.status.setText(f"移行画面を開けない: {exc}")
            return
        dialog.exec()
        if dialog.committed:
            self.status.setText(f"保存先を切り替えました: {destination}")
        self.refresh_storage_paths()
    ...
    def update_ai_text_engine_fields(self) -> None:
        ...
        # ClaudeCodeもGrokも --effort を受けるので、CLI系なら常に出す
        self.ai_cli_effort_label.setVisible(cli_mode)
        self.codex_exec_effort.setVisible(cli_mode)
        model_combo = self.ai_api_model if api_mode else self.codex_exec_model
        current_model = model_combo.currentText().strip()
        choices = ai_model_choices_for_engine(engine)
        model_combo.blockSignals(True)
        model_combo.clear()
        model_combo.addItems([""] + choices)
        # 別エンジンのモデル名が残ると誤ったCLIへ渡るので、その時だけ空へ戻す
        model_combo.setCurrentText(
            "" if ai_model_belongs_to_other_engine(engine, current_model) else current_model
        )
        model_combo.blockSignals(False)
        if api_mode and not self.ai_api_model.currentText().strip():
            self.ai_api_model.setCurrentText(
                "gpt-4o" if engine == "openai" else "gemini-2.5-flash"
            )
        self.refresh_ai_text_effort_choices()
        if cli_mode:
    ...
    def refresh_ai_text_effort_choices(self) -> None:
        engine = str(self.ai_text_engine.currentData() or "codex_exec")
        model = (
            self.ai_api_model if engine in {"openai", "gemini"} else self.codex_exec_model
        ).currentText()
        current = self.codex_exec_effort.currentText().strip()
        choices = ai_effort_choices_for(engine, model)
        self.codex_exec_effort.blockSignals(True)
        self.codex_exec_effort.clear()
        self.codex_exec_effort.addItems([""] + choices)
        # 推論は決まった値しか受け付けないので、対応しなければ空へ戻す
        self.codex_exec_effort.setCurrentText(current if current in choices else "")
        self.codex_exec_effort.blockSignals(False)
    ...
    def update_suno_provider_fields(self) -> None:
        subscription = str(self.suno_music_provider.currentData() or "api") == "subscription"
        self.suno_api_key.setEnabled(not subscription)
        self.refresh_suno_models_button.setEnabled(not subscription)
        self.open_suno_auth_browser_button.setEnabled(subscription)
        self.import_suno_auth_button.setEnabled(subscription)
        self.twocaptcha_api_key.setEnabled(subscription)
        if subscription:
            self.suno_auth_status.setText(
                "サブスク利用時はSunoログインと https://2captcha.com/enterpage のAPIキーが必要"
            )
        else:
            self.suno_auth_status.setText("Suno APIキーを使用")

    def open_suno_auth_browser(self) -> None:
        chrome = tracker.find_chrome_executable()
        profile = tracker.suno_cli_dir() / "chrome_profile"
        if chrome is None:
            self.suno_auth_status.setText("認証用Chromeが見つかりません")
            return
        ...
        self.suno_auth_status.setText(
            "Sunoへログイン後、「ログイン後に認証を取り込む」を押してください"
            if started else "認証用Chromeを起動できませんでした"
        )

    def import_suno_auth(self) -> None:
        if self.suno_auth_process and self.suno_auth_process.state() != QProcess.ProcessState.NotRunning:
            self.suno_auth_status.setText("Suno認証を取り込み中です")
            return
        python_exe = Path(sys.executable)
        auth_script = tracker.suno_cli_dir() / "auth_cdp.py"
        if not python_exe.is_file() or not auth_script.is_file():
            self.suno_auth_status.setText("Suno認証補助が見つかりません")
            return
        process = QProcess(self)
        process.setProgram(str(python_exe))
        process.setArguments([str(auth_script)])
        process.setWorkingDirectory(str(auth_script.parent))
        process.setProcessChannelMode(QProcess.ProcessChannelMode.MergedChannels)
        process.finished.connect(self.finish_suno_auth_import)
        self.suno_auth_process = process
        self.suno_auth_status.setText("Suno認証を取り込み中...")
        process.start()

    def finish_suno_auth_import(self, exit_code: int, _exit_status: QProcess.ExitStatus) -> None:
        status_path = tracker.suno_cli_dir() / "auth_cdp_status.json"
        message = "Sunoサブスク認証を取り込めませんでした"
        if exit_code == 0:
            message = "Sunoサブスク認証を取り込みました"
            ...
            try:
                ...
                pass
        self.suno_auth_status.setText(message)
        if self.suno_auth_process:
            self.suno_auth_process.deleteLater()
            self.suno_auth_process = None
    ...
    def save_recorder_settings(self) -> bool:
        ...
        if not path:
            ...
            return False
        target_root = self.target_root_path.text().strip()
        if not target_root:
            self.status.setText("保存できない: 生成物の保存先が空")
            return False
        if not self.save_recording_root():
            return False
        ...
        if not Path(path).exists():
            ...
            return False
        if (
            str(self.suno_music_provider.currentData() or "api") == "subscription"
            and not self.twocaptcha_api_key.text().strip()
        ):
            self.status.setText(
                "保存できない: Sunoサブスクには https://2captcha.com/enterpage のAPIキーが必要"
            )
            return False
        tracker.save_shared_env_value("TWOCAPTCHA_API_KEY", self.twocaptcha_api_key.text().strip())
        tracker.save_config_values(
            {
                ...
                "suno_music_provider": str(self.suno_music_provider.currentData() or "api"),
                ...
                "ai_text_model": self.ai_api_model.currentText().strip(),
                ...
                "codex_exec_model": self.codex_exec_model.currentText().strip(),
                "codex_exec_effort": self.codex_exec_effort.currentText().strip(),
...
class LinkedBroadcasterFilterModel(QAbstractTableModel):
    ...
    def set_only_checked(self, broadcaster_id: str) -> None:
        """指定した配信者だけをチェック状態にする。"""
        if not self._rows:
            return
        broadcaster_id = str(broadcaster_id or "").strip()
        for row in self._rows:
            row["visible"] = str(row.get("broadcaster_id") or "") == broadcaster_id
        top_left = self.index(0, 0)
        bottom_right = self.index(len(self._rows) - 1, 0)
        self.dataChanged.emit(top_left, bottom_right, [Qt.ItemDataRole.CheckStateRole])
...
class BroadcastTwoLineDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter: QPainter, option, index: QModelIndex) -> None:
        ...
        y = rect.top() + 4
        for part in parts:
            # ☑/☐ はON/OFFバッジ。「•」始まりはON/OFFではない情報バッジで、マークを付けない。
            info_only = part.startswith("•")
            enabled = part.startswith("☑")
            label = part[1:].strip()
            mark = "" if info_only else ("✓ " if enabled else "− ")
            width = metrics.horizontalAdvance(mark + label) + 20
    ...
    def data(self, index: QModelIndex, role: int = Qt.ItemDataRole.DisplayRole) -> Any:
        ...
        if role in (Qt.ItemDataRole.DisplayRole, Qt.ItemDataRole.ToolTipRole):
            if key == "visible":
                return ""
            value = row.get(key)
            if key in ("comment_count", "watch_count"):
                try:
                    count = int(value)
                except (TypeError, ValueError):
                    return ""
        ...
        self._rows = [
            {
                ...
                "broadcaster_id": str(row.get("broadcaster_id") or ""),
                "broadcaster_name": str(row.get("broadcaster_name") or ""),
                "generated_url": str(row.get("generated_url") or ""),
            }
            for row in rows
        ]
        self.endResetModel()
...
def install_special_broadcast_menu(table: QTableView, context) -> None:
    """スペシャルユーザーの放送履歴専用の右クリックメニュー。

    汎用メニュー(install_table_copy_menu)は配信者アーカイブ前提で、
    タグ編集・ローカル処理・録画停止など、このページに存在しない機能まで出る。
    LVだけを頼りに storage/html の別物HTMLを掴む問題もあるため、丸ごと差し替える。
    """
    table.setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)
    try:
    ...
    def add_copy(menu: QMenu, label: str, value: str) -> None:
        value = str(value or "").strip()
        if not value:
            return
        action = menu.addAction(label)
        action.triggered.connect(lambda _=False, text=value, name=label: copy_text(text, name))

    def add_open(menu: QMenu, label: str, url: str) -> None:
        url = str(url or "").strip()
        if not url:
            return
        action = menu.addAction(label)
        action.triggered.connect(lambda _=False, target=url: QDesktopServices.openUrl(QUrl(target)))

    def add_open_file(menu: QMenu, label: str, path: Path) -> None:
        action = menu.addAction(label)
        action.triggered.connect(
            lambda _=False, target=str(path): QDesktopServices.openUrl(
                QUrl.fromLocalFile(str(Path(target).resolve()))
            )
        )
    ...
    def show_menu(position) -> None:
        ...
        if not row:
            return
        lv = str(row.get("lv") or "").strip()
        user_id = str(context.special_combo.currentData() or "")
        menu = QMenu(table)

        cell_header = model.headerData(
        ...
        if cell_value not in (None, ""):
            ...
            copy_cell.triggered.connect(
                lambda _=False, text=str(cell_value): copy_text(text, "セルコピー")
            )

        if lv:
            menu.addSeparator()
            add_open(menu, "放送ページを開く", f"https://live.nicovideo.jp/watch/{lv}")

        # 複数選択したまま右クリックした場合は、選択分をまとめてローカル処理へ送る。
        # 2行で1放送を表示しているので、表示行ではなく元の放送単位で重複を潰す。
        selected_rows: list[dict[str, Any]] = []
        seen_sources: set[int] = set()
        for view_row in context_action_row_numbers(table, index.row()):
            source = model.source_row(view_row)
            if source in seen_sources:
                continue
            seen_sources.add(source)
            selected = model.row_at(view_row)
            if selected and str(selected.get("lv") or "").strip():
                selected_rows.append(selected)
        selected_lvs = [
            str(row.get("lv") or "").strip()
            for row in selected_rows
            if str(row.get("lv") or "").strip()
        ]
        if selected_lvs and user_id:
            menu.addSeparator()
            # 生成済みのものだけ公開、未生成のものだけタイムシフトGUIへ、と分けて出す。
            html_dir = tracker.special_user_html_dir(user_id)
            generated_lvs = [
                lv_value
                for lv_value in selected_lvs
                if (html_dir / f"{user_id}_{lv_value}_detail.html").is_file()
            ]
            pending_lvs = [lv_value for lv_value in selected_lvs if lv_value not in generated_lvs]
            if generated_lvs:
                label = (
                    "サーバーへアップロード"
                    if len(generated_lvs) == 1
                    else f"生成済み{len(generated_lvs)}放送をサーバーへアップロード"
                )
                upload = menu.addAction(label)
                upload.triggered.connect(
                    lambda _=False, targets=generated_lvs: context.upload_special_html(targets)
                )
            if pending_lvs:
                label = (
                    "未処理をタイムシフトGUIへ送る"
                    if len(pending_lvs) == 1
                    else f"未処理{len(pending_lvs)}放送をタイムシフトGUIへ送る"
                )
                send_pending = menu.addAction(label)
                send_pending.triggered.connect(
                    lambda _=False, targets=pending_lvs: context.send_special_pending_to_timeshift(targets)
                )

        # このユーザーのページ。配信者アーカイブHTMLとは別物なので専用に解決する。
        detail_path = tracker.special_user_html_dir(user_id) / f"{user_id}_{lv}_detail.html" if user_id and lv else None
        list_path = tracker.special_user_html_dir(user_id) / f"{user_id}_list.html" if user_id else None
        if detail_path is not None and detail_path.is_file():
            menu.addSeparator()
            add_open_file(menu, "個別HTMLを開く", detail_path)
            add_copy(menu, "個別HTMLのパスをコピー", str(detail_path))
        if list_path is not None and list_path.is_file():
            if detail_path is None or not detail_path.is_file():
                menu.addSeparator()
            add_open_file(menu, "一覧HTMLを開く", list_path)
            add_copy(menu, "一覧HTMLのパスをコピー", str(list_path))

        published_url = context.special_published_url(user_id, lv)
        list_url = context.special_list_published_url(user_id)
        if published_url or list_url:
            menu.addSeparator()
            add_open(menu, "生成ページにジャンプ", published_url)
            add_copy(menu, "生成ページURLをコピー", published_url)
            add_open(menu, "一覧ページにジャンプ", list_url)
            add_copy(menu, "一覧ページURLをコピー", list_url)

        menu.addSeparator()
        add_copy(menu, "LVをコピー", lv)
        add_copy(menu, "タイトルをコピー", row.get("title"))
        add_copy(menu, "配信者名をコピー", row.get("broadcaster_name"))
        add_copy(menu, "配信者IDをコピー", row.get("broadcaster_id"))
        broadcaster_id = str(row.get("broadcaster_id") or "").strip()
        if broadcaster_id:
            if broadcaster_id.startswith("ch"):
                slug = broadcaster_id[2:]
                broadcaster_url = f"https://ch.nicovideo.jp/{slug}" if slug else "https://ch.nicovideo.jp/"
            else:
                broadcaster_url = f"https://www.nicovideo.jp/user/{broadcaster_id}/live_programs"
            add_open(menu, "配信者の放送一覧を開く", broadcaster_url)

        menu.addSeparator()
        regenerate = menu.addAction("この放送のHTMLを作り直す")
        regenerate.triggered.connect(lambda _=False, target=lv: context.regenerate_special_html(target))
        regenerate.setEnabled(bool(lv and user_id))
...
class SpecialBroadcastFilterModel(BroadcastFilterModel):
    """スペシャルユーザー側の放送一覧。状態行に出すのは配信者アーカイブの要素ではなく、
    そのユーザーのページとして何が出来ているか。"""
    ...
    def data(self, index: QModelIndex, role: int = Qt.ItemDataRole.DisplayRole) -> Any:
        if (
            index.isValid()
            and self.is_detail_row(index.row())
            and not self.is_tag_row(index.row())
            and index.column() == 0
            and role in (Qt.ItemDataRole.DisplayRole, Qt.ItemDataRole.ToolTipRole)
        ):
            row = self._rows[self.source_row(index.row())]
            parts = [str(super().data(index, role) or "")]
            count = row.get("comment_count")
            if count not in (None, ""):
                parts.append(f"•発言 {count}件")
            name = str(row.get("broadcaster_name") or "").strip()
            if name:
                parts.append(f"•配信者: {name}")
...
class InspectionTab(QWidget):
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self.linked_broadcaster_filter_model.dataChanged.connect(
            lambda *_args: self.reload_special_broadcast_list()
        )
        ...
        self.linked_broadcaster_filter_table.doubleClicked.connect(self.show_only_this_broadcaster)
        ...
        self.broadcast_filter_model = SpecialBroadcastFilterModel()
        ...
        self.broadcast_filter_table.setShowGrid(False)
        self.broadcast_filter_table.setItemDelegate(
            BroadcastTwoLineDelegate(self.broadcast_filter_table)
        )
        self.broadcast_filter_model.modelReset.connect(
            lambda: QTimer.singleShot(0, self.broadcast_filter_table.resizeRowsToContents)
        )
        self.broadcast_filter_table.setColumnWidth(0, 55)
        self.broadcast_filter_table.setColumnWidth(1, 110)
        self.broadcast_filter_table.setColumnWidth(2, 260)
        self.broadcast_filter_table.setColumnWidth(3, 150)
        self.broadcast_filter_table.clicked.connect(self.show_special_broadcast_detail)
        install_special_broadcast_menu(self.broadcast_filter_table, self)
        self.broadcast_check_all_button = QPushButton("全選択")
        ...
        self.special_status_visible = QCheckBox("状態を表示")
        self.special_status_visible.setChecked(True)
        self.special_status_visible.toggled.connect(self.broadcast_filter_model.set_show_details)
        self.special_uploaded_only = QCheckBox("HTML公開済みのみ")
        self.special_uploaded_only.setChecked(False)
        self.special_uploaded_only.toggled.connect(
            lambda _checked: self.apply_special_broadcast_filter()
        )
        self.broadcast_filter_title = QLabel("放送履歴")
        self.special_comments_view = QTextEdit()
        self.special_comments_view.setReadOnly(True)
        self.special_comments_view.setPlainText("放送を選ぶとコメントを表示します")
        self.special_analysis_view = QTextEdit()
        self.special_analysis_view.setReadOnly(True)
        self.special_analysis_view.setPlainText("放送を選ぶと分析を表示します")
        self.special_selected_lv = ""
        ...
        self.broadcaster_broadcast_filter_table.setColumnWidth(4, 90)
        self.broadcaster_broadcast_filter_table.setColumnWidth(5, 90)
        self.broadcaster_broadcast_filter_table.setColumnWidth(6, 90)
        ...
        special_left.setMinimumWidth(0)
        ...
        special_left_layout.setContentsMargins(0, 0, 0, 0)
        self.special_content_tabs = QTabWidget()
        linked_page = QWidget()
        linked_page_layout = QVBoxLayout(linked_page)
        linked_page_layout.setContentsMargins(0, 0, 0, 0)
        ...
        self.special_content_tabs.addTab(linked_page, "紐づいてる配信者一覧")
        special_comment_page = QWidget()
        special_comment_layout = QVBoxLayout(special_comment_page)
        special_comment_layout.setContentsMargins(0, 0, 0, 0)
        special_comment_layout.addWidget(self.special_comments_view, 1)
        self.special_content_tabs.addTab(special_comment_page, "コメント一覧")
        special_analysis_page = QWidget()
        special_analysis_layout = QVBoxLayout(special_analysis_page)
        special_analysis_layout.setContentsMargins(0, 0, 0, 0)
        special_analysis_layout.addWidget(self.special_analysis_view, 1)
        self.special_content_tabs.addTab(special_analysis_page, "分析")
        special_left_layout.addWidget(self.special_content_tabs, 1)
        special_right = QWidget()
        special_right.setMinimumWidth(420)
        special_right_layout = QVBoxLayout(special_right)
        special_right_layout.setContentsMargins(0, 0, 0, 0)
        special_right_layout.addWidget(self.broadcast_filter_title)
        special_right_layout.addWidget(self.special_broadcast_search)
        special_right_layout.addWidget(self.special_broadcast_date_controls)
        ...
        broadcast_button_row.addWidget(self.special_status_visible)
        broadcast_button_row.addWidget(self.special_uploaded_only)
        broadcast_button_row.addStretch(1)
        special_right_layout.addLayout(broadcast_button_row)
        special_right_layout.addWidget(self.broadcast_filter_table, 1)
        self.special_splitter.addWidget(special_left)
        self.special_splitter.addWidget(special_right)
        self.special_splitter.setSizes([900, 460])
    ...
    def send_broadcaster_rows_to_local_processing(
        ...
        # 元動画は処理後に削除されるため、動画なしでも止めない。
        # 前処理を伴わないstepだけならLV指定で再処理できる。
        if not paths and not missing_lvs:
            show_status(self, "ローカル処理へ送れる放送がありません", "WARN")
            return
        try:
            result = send_local_targets_to_processing_gui(paths, missing_lvs)
        except Exception as exc:
            show_status(
                self,
        ...
        message = f"ローカル処理へ{action}: {len(lvs)}放送"
        if paths:
            message += f" / 動画{len(paths)}件（{len(lvs) - len(missing_lvs)}放送）"
        if missing_lvs:
            message += f" / 動画なしLV指定{len(missing_lvs)}放送"
        show_status(self, message, "INFO")
    ...
    def reload(self) -> None:
        ...
        self.reload_special_broadcast_list()
    ...
    def show_only_this_broadcaster(self, index: QModelIndex) -> None:
        """ダブルクリックした配信者だけにチェックを絞る。右の放送履歴もそれに従う。"""
        broadcaster_id = self.linked_broadcaster_filter_model.broadcaster_id_at(index.row())
        if not broadcaster_id:
            return
        self.linked_broadcaster_filter_model.set_only_checked(broadcaster_id)

    def show_linked_broadcaster_list(self) -> None:
        self._special_broadcast_rows = []
        self.broadcast_filter_model.update_rows([])
        self.broadcast_filter_title.setText("放送履歴")
        self.clear_special_broadcast_detail()
        self.special_content_tabs.setCurrentIndex(0)

    def clear_special_broadcast_detail(self) -> None:
        self.special_selected_lv = ""
        self.special_comments_view.setPlainText("放送を選ぶとコメントを表示します")
        self.special_analysis_view.setPlainText("放送を選ぶと分析を表示します")

    def show_special_broadcast_detail(self, index: QModelIndex) -> None:
        """右の放送履歴で選んだ放送のコメントと分析を、左のタブに出す。"""
        row = self.broadcast_filter_model.row_at(index.row())
        lv = str(row.get("lv") or "").strip()
        if not lv:
            return
        self.special_selected_lv = lv
        title = str(row.get("title") or "").strip()
        user_id = str(self.special_combo.currentData() or "")
        with tracker.connect() as conn:
            comment_rows = conn.execute(
                """
                SELECT broadcast_seconds, user_id, user_name, text
                FROM archive_comments
                WHERE lv = ?
        ...
        for item in comment_rows:
            is_own = str(item["user_id"] or "") == user_id
            if is_own:
                own_count += 1
            mark = "★ " if is_own else "  "
            name = str(item["user_name"] or item["user_id"] or "匿名")
            lines.append(f"{mark}[{clock(item['broadcast_seconds'])}] {name}: {str(item['text'] or '').strip()}")
        header = f"{lv} / {title or 'タイトル未取得'} / 全{len(comment_rows)}件 / ★=本人 {own_count}件"
        self.special_comments_view.setPlainText(
            f"{header}\n\n" + ("\n".join(lines) if lines else "コメントなし")
        )
        self.special_analysis_view.setPlainText(
            f"{header}\n\n{self.load_special_analysis_text(user_id, lv)}"
        )

    def load_special_analysis_text(self, user_id: str, lv: str) -> str:
        """生成済みの個別HTMLから分析部分を取り出す。無ければ未生成と返す。"""
        if not user_id or not lv:
            return "分析なし"
        detail_path = tracker.special_user_html_dir(user_id) / f"{user_id}_{lv}_detail.html"
        if not detail_path.is_file():
            return f"HTML未生成: {detail_path}"
        try:
            html_text = detail_path.read_text(encoding="utf-8", errors="ignore")
        except OSError as exc:
            return f"HTML読込失敗: {exc}"
        marker = "コメント分析結果"
        position = html_text.find(marker)
        if position < 0:
            return "分析セクションが見つかりません"
        body = html_text[position + len(marker):]
        body = re.sub(r"<br\s*/?>", "\n", body, flags=re.IGNORECASE)
        body = re.sub(r"</(p|div|h\d|tr)>", "\n", body, flags=re.IGNORECASE)
        body = re.sub(r"<[^>]+>", "", body)
        body = unescape(body)
        lines = [line.strip() for line in body.splitlines()]
        text = "\n".join(line for line in lines if line)
        return text.strip() or "分析なし"

    def reload_special_broadcast_list(self) -> None:
        """右の放送履歴は、左でチェックが入っている配信者すべての放送を出す。"""
        user_id = str(self.special_combo.currentData() or "")
        broadcaster_ids = [
            str(value).strip()
            for value in (self.linked_broadcaster_filter_model.checked_broadcaster_ids() or [])
            if str(value).strip()
        ]
        if not user_id or not broadcaster_ids:
            self._special_broadcast_rows = []
            self.broadcast_filter_model.update_rows([])
            self.broadcast_filter_title.setText("放送履歴")
            self.clear_special_broadcast_detail()
            return
        placeholders = ",".join("?" for _ in broadcaster_ids)
        params: list[str] = []
        for _ in range(3):
            params.append(user_id)
            params.extend(broadcaster_ids)
        with tracker.connect() as conn:
            rows = conn.execute(
                f"""
                SELECT lv,
                       ...
                       MAX(html_uploaded_at) AS html_uploaded_at,
                       MAX(broadcaster_name) AS broadcaster_name,
                       MAX(broadcaster_id) AS broadcaster_id
                FROM (
                    SELECT h.lv,
                           COALESCE(m.title, b.title, '') AS title,
                           COALESCE(h.broadcaster_name, m.broadcaster_name, b.broadcaster_name, h.broadcaster_id) AS broadcaster_name,
                           h.broadcaster_id AS broadcaster_id,
                           h.first_seen_at AS detected_at,
                           h.html_uploaded_at AS html_uploaded_at,
                    ...
                    WHERE h.user_id = ?
                      AND h.broadcaster_id IN ({placeholders})
                    UNION ALL
                    SELECT h.lv,
                           COALESCE(m.title, b.title, '') AS title,
                           COALESCE(h.broadcaster_name, m.broadcaster_name, b.broadcaster_name, h.broadcaster_id) AS broadcaster_name,
                    ...
                    SELECT c.lv,
                           COALESCE(m.title, b.title, '') AS title,
                           COALESCE(m.broadcaster_name, b.broadcaster_name, '') AS broadcaster_name,
                           COALESCE(m.broadcaster_id, b.broadcaster_id, '') AS broadcaster_id,
                           c.posted_at AS detected_at,
                           NULL AS html_uploaded_at,
                    ...
                    LEFT JOIN broadcasts b ON b.lv = c.lv
                    WHERE c.user_id = ?
                      AND COALESCE(m.broadcaster_id, b.broadcaster_id, '') IN ({placeholders})
                )
                WHERE COALESCE(lv, '') <> ''
                GROUP BY lv
            ...
            comment_counts = {
                str(count_row["lv"]): int(count_row["comment_count"] or 0)
                for count_row in conn.execute(
                    """
                    SELECT lv, COUNT(*) AS comment_count
                    FROM archive_comments
                    WHERE user_id = ?
                    GROUP BY lv
                    """,
        ...
        names = [
            str(row.get("broadcaster_name") or row.get("broadcaster_id") or "")
            for row in self.linked_broadcaster_filter_model._rows
            if str(row.get("broadcaster_id") or "") in set(broadcaster_ids)
        ]
        if len(names) <= 3:
            selection_text = " / ".join(name for name in names if name)
        else:
            selection_text = f"{len(names)}配信者"
        self.broadcast_filter_title.setText(
            f"放送履歴: {selection_text} / {len(rows)}放送"
        )
        listed_lvs = self.special_list_page_lvs(user_id)
        with tracker.connect() as conn:
            uploaded_lvs = tracker.special_user_uploaded_lvs(conn, user_id)
        self._special_broadcast_rows = []
        for row in rows:
            lv = str(row["lv"] or "")
            elements = self.special_generated_elements(user_id, lv, listed_lvs)
            self._special_broadcast_rows.append(
                {
                    "visible": True,
                    "lv": row["lv"],
                    "title": row["title"],
                    ...
                    "html_uploaded": "済"
                    if (lv in uploaded_lvs or str(row["html_uploaded_at"] or "").strip())
                    else "未",
                    "generated_elements": " / ".join(elements) if elements else "なし",
                    "comment_count": comment_counts.get(lv, 0),
        ...
        self.select_first_special_broadcast()

    def special_html_base_url(self, user_id: str) -> str:
        """公開先のベースURL。ユーザー設定が空なら既定の置き場所を使う。"""
        user_id = str(user_id or "").strip()
        if not user_id:
            return ""
        with tracker.connect() as conn:
            row = conn.execute(
                "SELECT html_base_url FROM special_users WHERE user_id = ?",
                (user_id,),
            ).fetchone()
        base = str(row["html_base_url"] or "").strip().rstrip("/") if row else ""
        if base:
            return base
        return f"https://warehouse.bitter.jp/niconico/{quote(user_id)}/special_user"

    def special_published_url(self, user_id: str, lv: str) -> str:
        """個別ページの公開URL。実際に公開した記録があるときだけ出す。"""
        user_id = str(user_id or "").strip()
        lv = str(lv or "").strip()
        if not user_id or not lv:
            return ""
        if not tracker.special_user_html_uploaded(user_id, lv):
            return ""
        return f"{self.special_html_base_url(user_id)}/{quote(user_id)}_{quote(lv)}_detail.html"

    def special_list_published_url(self, user_id: str) -> str:
        """一覧ページの公開URL。1件でも公開済みなら一覧も上がっている。"""
        user_id = str(user_id or "").strip()
        if not user_id:
            return ""
        with tracker.connect() as conn:
            if not tracker.special_user_uploaded_lvs(conn, user_id):
                return ""
        return f"{self.special_html_base_url(user_id)}/{quote(user_id)}_list.html"

    def send_special_pending_to_timeshift(self, lvs: list[str]) -> None:
        """HTML未生成の放送を、タイムシフトGUIの「スペシャル処理」へ渡す。"""
        user_id = str(self.special_combo.currentData() or "")
        jobs = [
            {"user_id": user_id, "lv": str(value).strip()}
            for value in (lvs or [])
            if str(value).strip()
        ]
        if not user_id or not jobs:
            return
        try:
            result = send_special_user_jobs_to_timeshift_gui(jobs)
        except Exception as exc:
            append_app_log(traceback.format_exc(), "DEBUG")
            show_status(self, f"タイムシフトGUI送信失敗: {type(exc).__name__}: {exc}", "ERROR")
            return
        action = "起動して送信" if result == "started" else "送信"
        show_status(self, f"タイムシフトGUIへ{action}: {len(jobs)}放送")

    def upload_special_html(self, lvs: list[str]) -> None:
        """選択した放送分の、生成済みHTMLをサーバーへ公開する。"""
        user_id = str(self.special_combo.currentData() or "")
        lvs = [str(value).strip() for value in (lvs or []) if str(value).strip()]
        if not user_id or not lvs:
            return
        show_status(self, f"スペシャルユーザーHTML公開開始: {user_id} / {len(lvs)}放送")
        job = SpecialUploadJob(user_id, lvs)
        job.signals.finished.connect(self.on_special_upload_finished)
        job.signals.failed.connect(
            lambda target, error: show_status(self, f"HTML公開失敗: {target} / {error}", "ERROR")
        )
        QThreadPool.globalInstance().start(job)

    def on_special_upload_finished(self, user_id: str, result: object) -> None:
        data = dict(result) if isinstance(result, dict) else {}
        if not data.get("uploaded"):
            show_status(self, f"HTML公開できず: {data.get('reason')}", "ERROR")
            append_app_log(f"スペシャルユーザーHTML公開失敗: {user_id} / {data}", "WARN")
            return
        show_status(
            self,
            f"スペシャルユーザーHTML公開: {user_id} / {len(data.get('lvs') or [])}放送 "
            f"/ {data.get('files')}ファイル → {data.get('remote_directory')}",
        )
        missing = data.get("missing_lvs") or []
        if missing:
            append_app_log(f"個別HTML未生成のため除外: {', '.join(missing)}", "WARN")
        self.reload_special_broadcast_list()

    def regenerate_special_html(self, lv: str) -> None:
        """右クリックからの手動生成。放送終了時と同じ処理を単発で走らせる。"""
        lv = str(lv or "").strip()
        if not lv:
            return
        job = SpecialUserHtmlJob(lv, force_overwrite=True)
        job.signals.finished.connect(self.on_special_html_regenerated)
        job.signals.failed.connect(
            lambda target, error: show_status(self, f"HTML生成失敗: {target} / {error}", "ERROR")
        )
        show_status(self, f"スペシャルユーザーHTML生成開始: {lv}")
        QThreadPool.globalInstance().start(job)

    def on_special_html_regenerated(self, lv: str, results: object) -> None:
        rows = list(results) if isinstance(results, list) else []
        if not rows:
            show_status(
                self,
                f"HTML生成対象なし: {lv} / 「HTMLを生成する」がONのユーザーにヒットがありません",
                "WARN",
            )
            return
        ok = sum(1 for row in rows if row.get("status") == "generated")
        show_status(self, f"スペシャルユーザーHTML生成: {lv} / 成功 {ok} / 全{len(rows)}")
        self.reload_special_broadcast_list()

    def special_list_page_lvs(self, user_id: str) -> set[str]:
        """一覧HTMLに載っているLVを集める。載っていれば一覧から辿れる。"""
        list_path = tracker.special_user_html_dir(user_id) / f"{user_id}_list.html"
        ...
        try:
            ...
            return set()
        return set(re.findall(r"lv\d+", text))

    def special_generated_elements(self, user_id: str, lv: str, listed_lvs: set[str]) -> list[str]:
        """そのユーザーのそのLV向けに、何が出来ているかを返す。"""
        elements: list[str] = []
        detail_path = tracker.special_user_html_dir(user_id) / f"{user_id}_{lv}_detail.html"
        if not detail_path.is_file():
            return elements
        elements.append("個別HTML")
        if lv in listed_lvs:
            elements.append("一覧HTML")
        analysis = self.load_special_analysis_text(user_id, lv)
        # 基本統計(総コメント数/平均文字数)だけの場合はAI分析なしとみなす
        body = re.sub(r"^-\s*(総コメント数|平均文字数).*$", "", analysis, flags=re.MULTILINE)
        if len(body.strip()) >= 80:
            elements.append("AI分析")
        return elements

    def select_first_special_broadcast(self) -> None:
        """配信者を選んだ直後は、先頭の放送を開いた状態にする。"""
        if self.broadcast_filter_model.rowCount() <= 0:
            self.clear_special_broadcast_detail()
            return
        index = self.broadcast_filter_model.index(0, 1)
        self.broadcast_filter_table.setCurrentIndex(index)
        self.show_special_broadcast_detail(index)
    ...
    def apply_special_broadcast_filter(self) -> None:
        ...
        for row in self._special_broadcast_rows:
            if self.special_uploaded_only.isChecked() and row.get("html_uploaded") != "済":
                continue
            if query and not (
    ...
    def reload_broadcaster_broadcast_filter(self) -> None:
        ...
        with tracker.connect() as conn:
            programs = conn.execute(
                ...
                SELECT m.lv,
                       ...
                       COALESCE(m.begin_time, json_extract(d.payload_json, '$.begin_time'), m.open_time, json_extract(d.payload_json, '$.open_time')) AS begin_value,
                       COALESCE(m.end_time, json_extract(d.payload_json, '$.end_time')) AS end_value,
                       COALESCE(
                           ac.comment_count,
                           NULLIF(CAST(json_extract(d.payload_json, '$.comment_count') AS INTEGER), 0),
                           b.comment_count
                       ) AS comment_count_value,
                       COALESCE(
                           m.watch_count,
                           NULLIF(CAST(json_extract(d.payload_json, '$.watch_count') AS INTEGER), 0),
                           b.watch_count
                       ) AS watch_count_value,
                       m.watch_url,
                ...
                LEFT JOIN broadcasts b ON b.lv = m.lv
                LEFT JOIN (
                    SELECT lv, COUNT(*) AS comment_count
                    FROM archive_comments
                    GROUP BY lv
        ...
        for row in program_dicts:
            ...
            self._broadcaster_broadcast_rows.append(
                ...
                "duration": broadcast_duration_text(row.get("begin_value"), row.get("end_value")),
                "comment_count": row.get("comment_count_value"),
                "watch_count": row.get("watch_count_value"),
                "time_value": row["time_value"],
    ...
    with tracker.connect() as conn:
        conn.execute(
            ...
            INSERT INTO special_users
                (user_id, label, note, analysis_prompt, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?)
...
class TimeshiftLocalFilesTab(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        # 元動画が削除済みの放送。LVだけで再処理する対象。
        self.lv_only: list[str] = []
        ...
        explanation = QLabel(
            ...
            "ファイル名に lv番号が必要です。\n"
            "監視アプリから送られた「動画なし（LV指定）」の放送も一覧に並びます。これは元動画が削除済みの放送の再処理用で、step01/step02は実行できません。"
        )
    ...
    def add_paths(self, paths) -> None:
        ...
        if added:
            ...
            self.refresh_list()
            ordered_lvs = self.ordered_target_lvs()
            self.append_status(f"処理順（古い→新しい）: {' → '.join(ordered_lvs)}")
            self.apply_broadcaster_step_defaults(ordered_lvs[0])

    @pyqtSlot(object)
    def add_lvs(self, lvs) -> None:
        """元動画が削除済みの放送を、LV指定だけで処理対象へ追加する。"""
        existing = set(self.lv_only) | {
            self.lv_from_path(path) for path in self.paths
        }
        added = False
        for raw_value in lvs or []:
            match = re.search(r"lv\d+", str(raw_value or ""), flags=re.IGNORECASE)
            if not match:
                self.append_status(f"除外（lv番号なし）: {raw_value}")
                continue
            lv = match.group(0).lower()
            if lv in existing:
                continue
            existing.add(lv)
            self.lv_only.append(lv)
            added = True
            self.append_status(f"追加（動画なし・LV指定）: {lv}")
        if added:
            self.sort_lv_only_oldest_first()
            self.refresh_list()
            ordered_lvs = self.ordered_target_lvs()
            self.append_status(f"処理順（古い→新しい）: {' → '.join(ordered_lvs)}")
            self.apply_broadcaster_step_defaults(ordered_lvs[0])

    def ordered_target_lvs(self) -> list[str]:
        return tracker.sort_broadcast_lvs_oldest_first(
            [self.lv_from_path(path) for path in self.paths] + list(self.lv_only)
        )

    def sort_lv_only_oldest_first(self) -> None:
        ordered = tracker.sort_broadcast_lvs_oldest_first(list(self.lv_only))
        positions = {lv: index for index, lv in enumerate(ordered)}
        self.lv_only.sort(key=lambda lv: positions.get(lv, len(positions)))
    ...
    def remove_selected(self) -> None:
        # 降順で消すので、LV指定行（動画行の後ろ）を先に処理できる。
        rows = sorted((self.drop_list.row(item) for item in self.drop_list.selectedItems()), reverse=True)
        video_count = len(self.paths)
        for row in rows:
            self.drop_list.takeItem(row)
            if 0 <= row < video_count:
                self.paths.pop(row)
                continue
            lv_index = row - video_count
            if 0 <= lv_index < len(self.lv_only):
                self.lv_only.pop(lv_index)
    ...
    def start_finalize(self) -> None:
        ...
        self.paths = retained
        self.sort_lv_only_oldest_first()
        for lv in self.lv_only:
            # 動画なしのLV指定。前処理なしのstepだけを回す。
            groups.setdefault(lv, [])
        self.refresh_list()
        if not groups:
            QMessageBox.information(self, "タイムシフト", "処理対象がありません。")
            return
        ...
        if run_preprocessing and self.lv_only:
            QMessageBox.information(
                self,
                "タイムシフト",
                "動画なし（LV指定）の放送が含まれています。\n"
                "step01とstep02は元動画が必要なため実行できません。\n"
                "両方のチェックを外してから開始してください。\n\n"
                f"対象: {', '.join(self.lv_only)}",
            )
            return
        self.append_status(
    ...
    def refresh_list(self) -> None:
        # 動画行を先、LV指定行を後ろに並べる。remove_selectedの添字計算がこの順序に依存する。
        self.drop_list.clear()
        for path in self.paths:
            lv = self.lv_from_path(path)
        ...
        for lv in self.lv_only:
            item = QListWidgetItem(f"{lv}  |  （動画なし / LV指定で再処理）")
    ...
    def finalize_finished(self, payload: dict[str, Any]) -> None:
        ...
        self.lv_only = [lv for lv in self.lv_only if lv not in successful_lvs]
        self.refresh_list()
...
class SpecialProcessTab(QWidget):
    """確認タブから送られた、未処理のスペシャルユーザー放送をここで生成する。"""
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        explanation = QLabel(
            "監視アプリのスペシャル確認から送られた、HTML未生成の放送を処理します。\n"
            "実行すると、その放送のスペシャルユーザーHTML(一覧・個別)を生成します。"
        )
        explanation.setWordWrap(True)

        self.model = PendingUploadModel()
        self.model.columns = [
            ("selected", "選択"),
            ("lv", "LV"),
            ("user_label", "スペシャルユーザー"),
            ("title", "タイトル"),
            ("begin_time_text", "開始"),
            ("status", "状態"),
        ]
        ...
        self.table.setAlternatingRowColors(True)
        self.table.verticalHeader().setVisible(False)
        configure_table_header(self.table, [55, 110, 180, 420, 150, 160])

        # スペシャルユーザーのページに関係するのはこの2つだけ。
        # 配信者側のようなStep一覧は構造が違うので持ち込まない。
        self.analysis_check = QCheckBox("AI分析を作る")
        self.analysis_check.setChecked(True)
        self.analysis_check.setToolTip("外すとコメント表と基本統計だけを作る。AI呼び出しをしないので速い")
        self.upload_check = QCheckBox("サーバーへアップロードする")
        self.upload_check.setChecked(False)
        self.upload_check.setToolTip("生成に成功した放送を、続けてサーバーへ公開する")
        self.force_check = QCheckBox("既存HTMLを作り直す")
        self.force_check.setChecked(False)
        self.force_check.setToolTip(
            "既にある個別HTMLは通常保持する。チェックすると上書きして作り直す"
        )
        self.option_box = QGroupBox("処理内容")
        option_layout = QHBoxLayout(self.option_box)
        option_layout.addWidget(QLabel("一覧・個別HTMLを作る"))
        option_layout.addWidget(self.analysis_check)
        option_layout.addWidget(self.force_check)
        option_layout.addWidget(self.upload_check)
        option_layout.addStretch(1)

        self.check_all_button = QPushButton("全選択")
        self.check_all_button.clicked.connect(lambda: self.model.set_all_selected(True))
        self.uncheck_all_button = QPushButton("全解除")
        self.uncheck_all_button.clicked.connect(lambda: self.model.set_all_selected(False))
        self.clear_button = QPushButton("一覧をクリア")
        self.clear_button.clicked.connect(self.clear_rows)
        self.run_button = QPushButton("選択を処理")
        self.run_button.clicked.connect(self.start_process)
        self.count_label = QLabel("未処理 0件")
    ...
    def clear_rows(self) -> None:
        ...
        self.count_label.setText("未処理 0件")

    def add_jobs(self, jobs: list[dict[str, str]]) -> None:
        """確認タブから受け取ったLVを一覧に積む。既にある行は重複させない。"""
        existing = {(row["user_id"], row["lv"]) for row in self._rows}
        added = 0
        with tracker.connect() as conn:
            for job in jobs:
                user_id = str(job.get("user_id") or "").strip()
                lv = str(job.get("lv") or "").strip()
                if not user_id or not lv or (user_id, lv) in existing:
                    continue
                meta = conn.execute(
                    """
                    SELECT COALESCE(m.title, '') AS title,
                           CASE WHEN m.begin_time IS NOT NULL
                                THEN datetime(m.begin_time, 'unixepoch', 'localtime')
                                ELSE '' END AS begin_time_text
                    FROM broadcast_archive_meta m
                    WHERE m.lv = ?
                    """,
                ...
                label_row = conn.execute(
                    "SELECT COALESCE(label, user_id) AS label FROM special_users WHERE user_id = ?",
                    (user_id,),
                ).fetchone()
                self._rows.append(
                    {
                        ...
                        "lv": lv,
                        "user_id": user_id,
                        "user_label": f"{label_row['label'] if label_row else user_id} / {user_id}",
                        "title": str(meta["title"]) if meta else "",
                        "begin_time_text": str(meta["begin_time_text"]) if meta else "",
                        "status": "未処理",
                    }
                )
                existing.add((user_id, lv))
                added += 1
        self.model.update_rows(self._rows)
        self.count_label.setText(f"未処理 {len(self._rows)}件")
        self.append_log(f"{added}件を受け取りました")

    def start_process(self) -> None:
        if self.running:
            show_status(self, "処理中です", "WARN")
            return
        targets = self.model.selected_rows()
        if not targets:
            show_status(self, "処理する放送を選択してください", "WARN")
            return
        self.running = True
        self.run_button.setEnabled(False)
        include_analysis = self.analysis_check.isChecked()
        upload_after = self.upload_check.isChecked()
        force_overwrite = self.force_check.isChecked()
        self.append_log(
            f"{len(targets)}件の生成を開始 / AI分析={'あり' if include_analysis else 'なし'}"
            f" / 既存HTML={'作り直す' if force_overwrite else '保持'}"
            f" / アップロード={'する' if upload_after else 'しない'}"
        )
        job = SpecialProcessJob(
            targets,
        ...
        job.signals.row_status.connect(self.model.set_status)
    ...
    def on_finished(self, result: object) -> None:
        ...
        self.append_log(
            f"完了: 生成 {len(generated)} / 公開 {len(uploaded)} / 失敗 {len(failed)}"
        )
        for detail in failed:
            self.append_log(f"  失敗 {detail}")
        show_status(
            self,
            f"スペシャルユーザー処理: 生成 {len(generated)} / 公開 {len(uploaded)} / 失敗 {len(failed)}",
        )


class SpecialProcessJob(QRunnable):
    def __init__(
        self,
        ...
        *,
        ...
        self.force_overwrite = force_overwrite
        self.signals = PendingUploadSignals()

    def run(self) -> None:
        generated: list[str] = []
        failed: list[str] = []
        uploaded_lvs: list[str] = []
        # 同じユーザーの放送はまとめて公開する。一覧HTMLを何度も上げずに済む。
        pending_upload: dict[str, list[str]] = {}
        try:
            for position, target in enumerate(self.targets, start=1):
                lv = str(target.get("lv") or "").strip()
                user_id = str(target.get("user_id") or "").strip()
                if not lv:
                    continue
                self.signals.progress.emit(f"[{position}/{len(self.targets)}] {lv} 生成開始")
                self.signals.row_status.emit(lv, "処理中")
                try:
                    results = tracker.generate_special_user_html_for_lv(
                        lv,
                        include_analysis=self.include_analysis,
                        force_overwrite_existing_html=self.force_overwrite,
                    )
                    ok = [row for row in results if row.get("status") in {"generated", "preserved"}]
                    preserved = [row for row in results if row.get("status") == "preserved"]
                    if not results:
                        self.signals.row_status.emit(lv, "対象なし")
                        self.signals.progress.emit(
                            f"{lv} 対象なし(HTML生成ONのユーザーにヒットがありません)"
                        )
                        failed.append(f"{lv}: 対象なし")
                        continue
                    if not ok:
                        error = "; ".join(str(row.get("error") or "") for row in results)
                        raise RuntimeError(error or "生成失敗")
                    generated.append(lv)
                    if preserved and len(preserved) == len(ok):
                        self.signals.row_status.emit(lv, "既存HTMLを保持")
                        self.signals.progress.emit(f"{lv} 既存の個別HTMLを保持(作り直しなし)")
                    else:
                        self.signals.row_status.emit(lv, "生成済み")
                        self.signals.progress.emit(f"{lv} 生成完了({len(ok)}ユーザー)")
                    if self.upload_after:
                        for row in ok:
                            owner = str(row.get("user_id") or user_id).strip()
                            if owner:
                                pending_upload.setdefault(owner, []).append(lv)
                except Exception as exc:
                    detail = f"{type(exc).__name__}: {exc}"
                    failed.append(f"{lv}: {detail}")
                    self.signals.row_status.emit(lv, "失敗")
                    self.signals.progress.emit(f"{lv} 失敗: {detail}")
                    append_app_log(traceback.format_exc(), "DEBUG")

            for owner, lvs in pending_upload.items():
                self.signals.progress.emit(f"{owner}: {len(lvs)}放送を公開開始")
                for lv in lvs:
                    self.signals.row_status.emit(lv, "公開中")
                try:
                    result = tracker.upload_special_user_html(
                        owner, lvs, progress_callback=self.signals.progress.emit
                    )
                except Exception as exc:
                    result = {"uploaded": False, "reason": f"{type(exc).__name__}: {exc}"}
                    append_app_log(traceback.format_exc(), "DEBUG")
                if result.get("uploaded"):
                    uploaded_lvs.extend(lvs)
                    for lv in lvs:
                        self.signals.row_status.emit(lv, "公開済み")
                else:
                    reason = str(result.get("reason") or "不明")
                    failed.append(f"{owner} 公開: {reason}")
                    self.signals.progress.emit(f"{owner} 公開失敗: {reason}")
                    for lv in lvs:
                        self.signals.row_status.emit(lv, "生成済み(公開失敗)")
            self.signals.finished.emit(
                {"generated": generated, "failed": failed, "uploaded": uploaded_lvs}
            )
        except Exception as exc:
            append_app_log(traceback.format_exc(), "DEBUG")
            self.signals.finished.emit(
                {"generated": generated, "failed": [str(exc)], "uploaded": uploaded_lvs}
            )


class PendingUploadModel(QAbstractTableModel):
    columns = [
        ("selected", "選択"),
        ("lv", "LV"),
        ("broadcaster_name", "配信者"),
    ...
    def data(self, index: QModelIndex, role: int = Qt.ItemDataRole.DisplayRole) -> Any:
        ...
        if key == "selected" and role == Qt.ItemDataRole.CheckStateRole:
            return Qt.CheckState.Checked if row.get("selected") else Qt.CheckState.Unchecked
        if role not in (Qt.ItemDataRole.DisplayRole, Qt.ItemDataRole.ToolTipRole):
            return None
        if key == "selected":
            return ""
    ...
    def setData(self, index: QModelIndex, value: Any, role: int = Qt.ItemDataRole.EditRole) -> bool:
        if not index.isValid() or role != Qt.ItemDataRole.CheckStateRole:
            return False
        if self.columns[index.column()][0] != "selected":
            return False
        checked_value = value.value if isinstance(value, Qt.CheckState) else value
        self._rows[index.row()]["selected"] = checked_value == Qt.CheckState.Checked.value
        self.dataChanged.emit(index, index, [Qt.ItemDataRole.CheckStateRole])
        return True
    ...
    def selected_rows(self) -> list[dict[str, Any]]:
        return [dict(row) for row in self._rows if row.get("selected")]
    ...
    def set_status(self, lv: str, status: str) -> None:
        for position, row in enumerate(self._rows):
            if str(row.get("lv") or "") == lv:
                row["status"] = status
                index = self.index(position, len(self.columns) - 1)
                self.dataChanged.emit(index, index, [Qt.ItemDataRole.DisplayRole])
                return


class PendingUploadSignals(QObject):
    progress = pyqtSignal(str)
    row_status = pyqtSignal(str, str)
    finished = pyqtSignal(object)


class PendingUploadJob(QRunnable):
    """未アップロードのHTMLを、step15をLV単位で走らせて公開する。"""
    ...
    def run(self) -> None:
        ...
        try:
            for position, target in enumerate(self.targets, start=1):
                lv = str(target.get("lv") or "").strip()
                account_id = str(target.get("broadcaster_id") or "").strip()
                if not lv:
                    continue
                self.signals.progress.emit(f"[{position}/{len(self.targets)}] {lv} 公開開始")
                self.signals.row_status.emit(lv, "アップロード中")
                try:
                    ...
                    step = (result.get("steps") or {}).get("step15_lolipop_uploader") or {}
                    if step.get("status") == "error":
                        raise RuntimeError(str(step.get("error") or "step15失敗"))
                    payload = step.get("result") or {}
                    if isinstance(payload, dict) and payload.get("uploaded") is False:
                        reason = str(payload.get("reason") or "不明")
                        self.signals.row_status.emit(lv, f"スキップ: {reason}")
                        self.signals.progress.emit(f"{lv} スキップ: {reason}")
                        failed.append(f"{lv}: {reason}")
                        continue
                    uploaded.append(lv)
                    self.signals.row_status.emit(lv, "公開済み")
                    self.signals.progress.emit(f"{lv} 公開完了")
                except Exception as exc:
                    detail = f"{type(exc).__name__}: {exc}"
                    failed.append(f"{lv}: {detail}")
                    self.signals.row_status.emit(lv, "失敗")
                    self.signals.progress.emit(f"{lv} 失敗: {detail}")
                    append_app_log(traceback.format_exc(), "DEBUG")
            self.signals.finished.emit({"ok": True, "uploaded": uploaded, "failed": failed})
        except Exception as exc:
            append_app_log(traceback.format_exc(), "DEBUG")
            self.signals.finished.emit({"ok": False, "error": f"{type(exc).__name__}: {exc}"})


class PendingUploadTab(QWidget):
    """HTMLは出来ているがサーバーへ公開していない放送を一覧し、まとめて公開する。"""
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        explanation = QLabel(
            "生成済みHTMLのうち、サーバーへ未公開のものを表示します。\n"
            "選択してアップロードすると、その放送のStep15(アーカイブ公開)だけを実行します。"
        )
        explanation.setWordWrap(True)

        self.search_input = QLineEdit()
        self.search_input.setPlaceholderText("LV / 配信者 / タイトルで絞り込み")
        self.search_input.textChanged.connect(lambda *_args: self.apply_filter())
        ...
        self.table.setAlternatingRowColors(True)
        self.table.verticalHeader().setVisible(False)
        configure_table_header(self.table, [55, 110, 150, 420, 150, 150])
        self.table.doubleClicked.connect(self.open_html)
        ...
        self.uncheck_all_button = QPushButton("全解除")
        self.uncheck_all_button.clicked.connect(lambda: self.model.set_all_selected(False))
        self.upload_button = QPushButton("選択をアップロード")
        self.upload_button.clicked.connect(self.start_upload)
        self.count_label = QLabel("未公開 0件")
        ...
        with tracker.connect() as conn:
            rows = conn.execute(
                ...
                SELECT m.lv,
                       ...
                       COALESCE(NULLIF(m.broadcaster_id, ''), NULLIF(j.broadcaster_id, ''), '') AS broadcaster_id,
                       COALESCE(m.broadcaster_name, '') AS broadcaster_name,
                       COALESCE(m.html_path, '') AS html_path,
                       CASE WHEN m.begin_time IS NOT NULL
                            THEN datetime(m.begin_time, 'unixepoch', 'localtime')
                            ELSE '' END AS begin_time_text
                FROM broadcast_archive_meta m
                LEFT JOIN recording_jobs j ON j.lv = m.lv
                WHERE COALESCE(m.html_path, '') <> ''
                  AND COALESCE(m.archive_upload_completed, 0) = 0
                ORDER BY m.begin_time DESC, m.lv DESC
                LIMIT 1000
                """
            ).fetchall()
        self._rows = []
        for row in rows:
            html_path = Path(str(row["html_path"] or ""))
            ...
            self._rows.append(
                {
                    ...
                    "broadcaster_id": str(row["broadcaster_id"] or ""),
                    "broadcaster_name": str(row["broadcaster_name"] or row["broadcaster_id"] or ""),
                    "begin_time_text": str(row["begin_time_text"] or ""),
    ...
    def apply_filter(self) -> None:
        ...
        rows = [
            ...
            for row in self._rows
            if not query
            or query in str(row.get("lv") or "").lower()
            or query in str(row.get("broadcaster_name") or "").lower()
            or query in str(row.get("title") or "").lower()
        ]
        self.model.update_rows(rows)
        self.count_label.setText(f"未公開 {len(self._rows)}件 / 表示 {len(rows)}件")

    def open_html(self, index: QModelIndex) -> None:
    ...
    def start_upload(self) -> None:
        if self.running:
            show_status(self, "アップロード実行中です", "WARN")
            return
        targets = self.model.selected_rows()
        if not targets:
            show_status(self, "アップロードする放送を選択してください", "WARN")
            return
        missing = [row for row in targets if not str(row.get("broadcaster_id") or "").strip()]
        if missing:
            lvs = ", ".join(str(row.get("lv")) for row in missing[:5])
            show_status(self, f"配信者IDが不明な放送は公開できません: {lvs}", "ERROR")
            return
        self.running = True
        self.upload_button.setEnabled(False)
        self.append_log(f"{len(targets)}件のアップロードを開始")
        job = PendingUploadJob(targets)
        job.signals.progress.connect(self.append_log)
        job.signals.row_status.connect(self.model.set_status)
        job.signals.finished.connect(self.on_finished)
        QThreadPool.globalInstance().start(job)
    ...
    def on_finished(self, result: object) -> None:
        ...
        data = dict(result) if isinstance(result, dict) else {}
        if not data.get("ok"):
            self.append_log(f"アップロード中断: {data.get('error')}")
            show_status(self, f"アップロード中断: {data.get('error')}", "ERROR")
            return
        uploaded = list(data.get("uploaded") or [])
        failed = list(data.get("failed") or [])
        self.append_log(f"完了: 成功 {len(uploaded)} / 失敗 {len(failed)}")
        for detail in failed:
            self.append_log(f"  失敗 {detail}")
        show_status(self, f"アップロード完了: 成功 {len(uploaded)} / 失敗 {len(failed)}")
        self.reload()
...
class DisappearedBroadcastUrlTab(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self.url_edit.setPlaceholderText(
            "https://example.com/niconico/12345678/lv123456789/lv123456789.html"
        )
...
class TimeshiftTagEditorTab(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self.broadcaster_edit = QLineEdit()
        self.broadcaster_edit.setPlaceholderText("配信者ID（例: 12345678）")
        ...
        self.transcript_search.setPlaceholderText("文字起こしを検索")
        self.transcript_search.returnPressed.connect(self.find_next_transcript)
        self.transcript_search.textChanged.connect(self.reset_transcript_search)
        self.transcript_search_previous_button = QPushButton("前へ")
        self.transcript_search_previous_button.clicked.connect(self.find_previous_transcript)
        self.transcript_search_next_button = QPushButton("次へ")
        self.transcript_search_next_button.clicked.connect(self.find_next_transcript)
        self.transcript_search_status = QLabel("")
        self.transcript_search_row = -1
        ...
        interval_controls.addWidget(QLabel("文字検索"))
        interval_controls.addWidget(self.transcript_search, 1)
        interval_controls.addWidget(self.transcript_search_previous_button)
        interval_controls.addWidget(self.transcript_search_next_button)
        interval_controls.addWidget(self.transcript_search_status)
    ...
    def reset_transcript_search(self) -> None:
        self.transcript_search_row = -1
        self.transcript_search_status.setText("")

    def transcript_search_matches(self) -> list[int]:
        ...
        return [
            ...
            if query in str(
                self.transcript_table.item(row, 1).text()
                if self.transcript_table.item(row, 1) is not None
                else ""
            ).casefold()
        ]

    def move_transcript_search(self, direction: int) -> None:
        matches = self.transcript_search_matches()
        if not self.transcript_search.text().strip():
            self.transcript_search_status.setText("検索語を入力")
            return
        if not matches:
            self.transcript_search_status.setText("0件")
            return
        current_row = self.transcript_search_row
        if current_row not in matches:
            target_index = 0 if direction > 0 else len(matches) - 1
        else:
            target_index = (matches.index(current_row) + direction) % len(matches)
        target_row = matches[target_index]
        self.transcript_search_row = target_row
        self.transcript_table.setCurrentCell(target_row, 1)
        self.transcript_table.selectRow(target_row)
        item = self.transcript_table.item(target_row, 1)
        if item is not None:
            self.transcript_table.scrollToItem(
                item, QAbstractItemView.ScrollHint.PositionAtCenter
            )
        self.transcript_search_status.setText(f"{target_index + 1}/{len(matches)}")

    def find_next_transcript(self) -> None:
        self.move_transcript_search(1)

    def find_previous_transcript(self) -> None:
        self.move_transcript_search(-1)
...
class TimeshiftTab(QWidget):
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self.url_input.setPlaceholderText(
            ...
            "https://www.nicovideo.jp/user/12345678/live_programs"
        )
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self.finalize_backlog_timer = QTimer(self)
        self.finalize_backlog_timer.setInterval(60000)
        self.finalize_backlog_timer.timeout.connect(self.scan_finalize_backlog)
        self.finalize_backlog_timer.start()
        QTimer.singleShot(3000, self.scan_finalize_backlog)
        ...
        self.archive_manager_window: ArchiveManagerWindow | None = None
        archive_action = QAction("アーカイブ管理", self)
        archive_action.setToolTip("録画と生成物の置き場を一覧して、消すものを選ぶ窓を開きます")
        archive_action.triggered.connect(self.open_archive_manager)
    ...
    def open_archive_manager(self) -> None:
        """アーカイブ管理の窓を出す。開いていれば前面へ戻すだけ。"""
        if self.archive_manager_window is None:
            self.archive_manager_window = ArchiveManagerWindow(self)
            self.archive_manager_window.destroyed.connect(self.on_archive_manager_closed)
            self.archive_manager_window.setAttribute(Qt.WidgetAttribute.WA_DeleteOnClose, True)
        self.archive_manager_window.show()
        self.archive_manager_window.raise_()
        self.archive_manager_window.activateWindow()

    def on_archive_manager_closed(self, *_args) -> None:
        self.archive_manager_window = None

    def scan_finalize_backlog(self) -> None:
        if getattr(self, "finalize_backlog_scanning", False):
            return
        self.finalize_backlog_scanning = True
        job = FinalizeBacklogScanJob()
        job.signals.finished.connect(self.on_finalize_backlog_scanned)
        QThreadPool.globalInstance().start(job)

    def on_finalize_backlog_scanned(self, enqueued: object) -> None:
        self.finalize_backlog_scanning = False
        rows = list(enqueued) if isinstance(enqueued, list) else []
        if not rows:
            return
        self.statusBar().showMessage(f"未処理放送をHTML生成キューへ登録: {len(rows)}件")
        for row in rows:
            append_app_log(
                f"未処理放送をHTML生成キューへ登録: {row.get('lv')} / "
                f"{row.get('broadcaster_name') or row.get('broadcaster_id')} / 終了={row.get('ended_at')}",
                "INFO",
            )
